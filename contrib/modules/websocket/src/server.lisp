...
(defstruct (websocket-handler (:constructor %make-websocket-handler))
...
(defun make-websocket-handler (&rest args)
  "Create a websocket handler with event callbacks"
  (apply #'%make-websocket-handler args))
...
(defun start-server (server)
  ...
  (let* ((options (websocket-server-options server))
    ...
    (let ((socket (net:tcp-bind (format nil "~A:~A" host port))))
...
(defun stop-server (server)
  ...
  (when (websocket-server-socket server)
    (net:tcp-shutdown (websocket-server-socket server))
...
(defun accept-connection (server)
  ...
  (let ((client-stream (net:tcp-accept (websocket-server-socket server))))
    (when client-stream
      ;; Handle connection in separate thread
      (thread:make-thread
       (lambda () (handle-client-connection server client-stream))
       :name (format nil "websocket-client-~A" (generate-connection-id))))))

(defun handle-client-connection (server client-stream)
  "Handle individual client connection"
  ;; Create a bidirectional stream from the socket handle
  (let ((stream (sb-sys:make-fd-stream (net:tcp-stream-handle client-stream)
                                       :input t :output t 
    ...
    (unwind-protect
      ...
      ;; Cleanup
      (when client-stream
        (net:tcp-shutdown client-stream)))))
...
(defun add-websocket-to-http-server (http-server websocket-server &key (path "/ws"))
  ...
  ;; For now, this is a placeholder - the HTTP server module would need
  ;; to expose handler management functionality for full integration
  (declare (ignore http-server path))
  ;; Register the WebSocket server's upgrade handler
  (websocket-upgrade-handler websocket-server))
