...
(deftest valid-upgrade-request
  ...
  (let ((request (req:make-request "GET" "/"
                                   :headers (map:from-pairs '(("Host" . "example.com")
                                                              ("Upgrade" . "websocket")
                                                              ("Connection" . "Upgrade")
...
(deftest invalid-upgrade-requests
  ...
  ;; Wrong method
  (let ((request (req:make-request "POST" "/"
                                   :headers (map:from-pairs '(("Upgrade" . "websocket")
                                                              ("Connection" . "Upgrade")
                                                              ("Sec-WebSocket-Key" . "dGhlIHNhbXBsZSBub25jZQ==")
                                                              ("Sec-WebSocket-Version" . "13"))))))
    (is-thrown (ws:websocket-handshake-error)
      (handshake:validate-websocket-request request)))
  
  ;; Missing Upgrade header
  (let ((request (req:make-request "GET" "/"
                                   :headers (map:from-pairs '(("Connection" . "Upgrade")
                                                              ("Sec-WebSocket-Key" . "dGhlIHNhbXBsZSBub25jZQ==")
                                                              ("Sec-WebSocket-Version" . "13"))))))
    (is-thrown (ws:websocket-handshake-error)
      (handshake:validate-websocket-request request)))
...
(deftest handshake-response-creation
  ...
  (let* ((request (req:make-request "GET" "/"
         ...
         (let ((headers (resp:response-headers response)))
           (is-equal (map:get headers "Upgrade") "websocket")
           (is-equal (map:get headers "Connection") "Upgrade")
           (is-equal (map:get headers "Sec-WebSocket-Accept") 
...
(deftest handshake-with-subprotocol
  ...
  (let* ((request (req:make-request "GET" "/"
    ...
    (let ((headers (resp:response-headers response)))
      (is-equal (map:get headers "Sec-WebSocket-Protocol") "chat"))))
...
(deftest client-upgrade-request
  ...
  (multiple-value-bind (request websocket-key)
    ...
    (is-equal (req:request-path request) "/test")
    
    (let ((headers (req:request-headers request)))
      (is-equal (map:get headers "Host") "example.com")
      (is-equal (map:get headers "Upgrade") "websocket")
      (is-equal (map:get headers "Connection") "Upgrade")
      (is-equal (map:get headers "Sec-WebSocket-Version") "13")
      (is-equal (map:get headers "Sec-WebSocket-Key") websocket-key))
...
(deftest client-upgrade-with-subprotocols
  ...
  (multiple-value-bind (request websocket-key)
    ...
    (let ((headers (req:request-headers request)))
      (is-equal (map:get headers "Sec-WebSocket-Protocol") "chat,superchat"))))
...
(deftest websocket-request-detection
  ...
  (let ((websocket-request (req:make-request "GET" "/"
                                             :headers (map:from-pairs
                                                      '(("Upgrade" . "websocket")
                                                        ("Connection" . "Upgrade")
                                                        ("Sec-WebSocket-Key" . "dGhlIHNhbXBsZSBub25jZQ==")
                                                        ("Sec-WebSocket-Version" . "13")))))
        (http-request (req:make-request "GET" "/"
                                        :headers (map:from-pairs '(("Accept" . "text/html"))))))
...
(deftest header-extraction
  ...
  (let ((request (req:make-request "GET" "/"
                                   :headers (map:from-pairs '(("Sec-WebSocket-Key" . "test-key")
...
(deftest case-insensitive-headers
  ...
  (let ((request (req:make-request "GET" "/"
                                   :headers (map:from-pairs
                                            '(("upgrade" . "websocket")      ; lowercase
                                              ("CONNECTION" . "UPGRADE")     ; uppercase
                                              ("Sec-WebSocket-Key" . "dGhlIHNhbXBsZSBub25jZQ==")
                                              ("Sec-WebSocket-Version" . "13"))))))
