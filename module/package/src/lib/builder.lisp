(defpackage :epsilon.lib.package.builder
  (:use
   :cl
   :epsilon.lib.package)
  (:local-nicknames
...
(in-package :epsilon.lib.package.builder)

;;;; ==========================================================================
;;;; Module Discovery
;;;; ==========================================================================

(defun discover-modules (&optional (root-path (uri:make-uri :scheme "file" :path "module/")))
  ...
  (let ((modules (map:make-map)))
    ...
    (when (fs:exists-p root-path)
      (dolist (entry (fs:list-contents root-path))
        (when (fs:dir-p entry)
          (let ((package-file (uri:merge entry "package.edn")))
...
(defun register-all-modules (&optional (root-path (uri:make-uri :scheme "file" :path "module/")))
