...
(lib:defshared %epoll-create "epoll_create" "libc" :int (size :int)
  :documentation "Create epoll file descriptor (deprecated)")

(lib:defshared %epoll-create1 "epoll_create1" "libc" :int (flags :int)
...
(lib:defshared %epoll-ctl "epoll_ctl" "libc" :int
  ...
  :documentation "Control interface for epoll file descriptor")
...
(defun pack-epoll-event (event buffer offset)
  "Pack epoll_event structure into foreign buffer"
  (let ((ptr (sb-alien:sap+ (sb-alien:alien-sap buffer) offset)))
    ;; events (4 bytes)
    (setf (sb-alien:sap-ref-32 ptr 0) (epoll-event-events event))
    ;; data (8 bytes at offset 8 due to padding)
    (setf (sb-alien:sap-ref-64 ptr 8) (epoll-event-data event))))

(defun unpack-epoll-event (buffer offset)
  "Unpack epoll_event structure from foreign buffer"
  (let ((ptr (sb-alien:sap+ (sb-alien:alien-sap buffer) offset)))
    (make-epoll-event
     :events (sb-alien:sap-ref-32 ptr 0)
     :data (sb-alien:sap-ref-64 ptr 8))))
...
(defun epoll-ctl (epfd operation fd event)
  ...
  (lib:with-foreign-memory ((event-buf :char :count (epoll-event-size)))
    (when event
      (pack-epoll-event event event-buf 0))
    (let ((result (%epoll-ctl epfd operation fd 
                              (if event event-buf (sb-alien:null-alien)))))
      (when (= result -1)
...
(defun epoll-wait (epfd max-events timeout)
  ...
  (lib:with-foreign-memory ((events-buf :char :count (* max-events (epoll-event-size))))
