...
(defun websocket-connection (&rest args) (apply #'conn:websocket-connection args))
...
(defun broadcast-text (server text &key path) 
  (server:broadcast-text server text :path path))
(defun broadcast-binary (server data &key path) 
  (server:broadcast-binary server data :path path))
