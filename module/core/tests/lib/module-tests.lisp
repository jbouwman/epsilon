...
(defpackage :epsilon.lib.module.tests
  (:use 
   :cl 
   :epsilon.test)
  (:local-nicknames
   (:module :epsilon.lib.module)))

(in-package :epsilon.lib.module.tests)
  ...
  (let ((form (macroexpand-1 
    ...
    (let ((defpackage-form (second form)))
      (let ((local-nicknames-clause 
        ...
        (is (member '(map epsilon.lib.map) (rest local-nicknames-clause) :test #'equal))
        (is (member '(str epsilon.lib.string) (rest local-nicknames-clause) :test #'equal))))))
  ...
  (let ((form (macroexpand-1 
    ...
    (let ((defpackage-form (second form)))
      ...
      (let ((import-clauses 
        ...
        (is (member '(:import-from epsilon.lib.map make-map get assoc) 
                   import-clauses :test #'equal))
        (is (member '(:import-from epsilon.lib.string split join) 
                   import-clauses :test #'equal))))))
    ...
    (let ((defpackage-form (second form)))
      ...
      (let ((local-nicknames-clause 
             (find-if (lambda (clause) 
                      ...
                      (cddr defpackage-form))))
        (is (member '(map epsilon.lib.map) (rest local-nicknames-clause) :test #'equal))
        (is (member '(json epsilon.lib.json) (rest local-nicknames-clause) :test #'equal)))
      ...
      (let ((import-clauses 
             (remove-if-not (lambda (clause) 
                              (and (consp clause) 
                                   (eq (first clause) :import-from)))
                            (cddr defpackage-form))))
        (is (member '(:import-from epsilon.lib.string split join trim) 
                   import-clauses :test #'equal))))))
...
(deftest module-integration-test
  ...
  (eval '(module:module test-integration
           :use (cl)
           :import ((seq epsilon.lib.sequence))
    ...
    (dotimes (i 1000)
      (macroexpand-1 
       '(module:module test-package
          ...
          :import ((map epsilon.lib.map)
                   (str epsilon.lib.string)
                   ((split join) from epsilon.lib.text))
