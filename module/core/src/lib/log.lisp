(defpackage epsilon.lib.log
  (:use cl)
  (:shadow #:log #:trace #:debug #:error #:warn)
  (:local-nicknames
   (#:map #:epsilon.lib.map)
...
(defun find-parent-logger (name)
  ...
  (let ((parts (str:split #\. name)))
    (loop for i from (1- (length parts)) downto 1
          for parent-name = (str:join #\. (subseq parts 0 i))
...
(defun configure-from-string (config-string)
  "Configure logging from string like 'debug:epsilon.lib.*,trace:epsilon.lib.yaml'"
  ;; Simplified implementation - just set root level for now
  (let* ((specs (str:split #\, config-string))
         (first-spec (first specs)))
    (when first-spec
      (let* ((parts (str:split #\: first-spec))
