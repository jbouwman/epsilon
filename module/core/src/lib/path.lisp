...
;;;;
;;;; This module provides cross-platform filesystem path manipulation with
;;;; proper handling of Windows drive letters, Unix absolute paths, and
;;;; platform-specific path separators. Includes filesystem queries.
;;;;
;;;; Key Features:
;;;; - Cross-platform path normalization (Windows/Unix)
;;;; - Path manipulation (join, parent, normalize, extension)
;;;; - Filesystem queries (exists, type, directory listing)
...
(defun temp-directory ()
  ...
  (make-path
   (or #+win32 (or #+sbcl (sb-ext:posix-getenv "TEMP") #-sbcl nil
                   #+sbcl (sb-ext:posix-getenv "TMP") #-sbcl nil
                   "C:\\Temp")
       #-win32 (or #+sbcl (sb-ext:posix-getenv "TMPDIR") #-sbcl nil
                   "/tmp"))))

(defun home-directory ()
  "Get user home directory as path"
  (make-path
   #+win32 (or #+sbcl (sb-ext:posix-getenv "USERPROFILE") #-sbcl nil
               (concatenate 'string 
                           (or #+sbcl (sb-ext:posix-getenv "HOMEDRIVE") #-sbcl nil "")
                           (or #+sbcl (sb-ext:posix-getenv "HOMEPATH") #-sbcl nil "")))
   #-win32 (or #+sbcl (sb-ext:posix-getenv "HOME") #-sbcl nil "/tmp")))
...
(defun make-file-uri (path-or-string)
  "Create a file:// URI structure for a path (for migration from epsilon.lib.uri)"
  (let ((path-str (cond
                   ((typep path-or-string 'path) (path-string path-or-string))
                   ((stringp path-or-string) path-or-string)
                   (t (string path-or-string)))))
    ;; Return path object instead of URI since we're moving away from URI
    (make-path path-str)))
