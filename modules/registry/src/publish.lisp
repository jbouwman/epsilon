...
(defun publish-package (package-info &key registry auth-token)
  ...
  (let* ((reg (or (and registry (get-registry registry))
    ...
    (let ((response (make-registry-request reg "/packages"
                                           :method :post
                                           :body (json:to-json package-info)
                                           :auth-token token)))
      (if (= (http:response-status response) 201)
          (json:from-json (http:response-body response))
...
(defun valid-checksum-format-p (checksum)
  ...
  (and (stringp checksum)
       (str:starts-with-p "sha256:" checksum)
...
(defun publish-package-internal (package-info auth-token)
  "Internal function for handling package publication on the server.

   PACKAGE-INFO - Package information map
   AUTH-TOKEN - Authentication token (already verified)

   Returns result map."
  (validate-package-info package-info)
  (let ((name (map:get package-info "name"))
        (version (map:get package-info "version")))
    ;; Check if version already exists
    (when (version-exists-p name version)
      (error "Version ~A already exists for package ~A" version name))
    ;; Store package metadata
    (store-package-version name version package-info)
    ;; Return success
    (map:make-map "status" "published"
                  "package" name
                  "version" version)))

(defun version-exists-p (name version)
  "Check if a package version already exists."
  (let ((versions (get-package-versions-internal name)))
    (member version versions :test #'string=)))

(defun store-package-version (name version info)
  "Store a new package version in the registry database."
