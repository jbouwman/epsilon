...
;;; Package storage (server-side)

(defvar *storage-directory* nil
  "Base directory for package storage on server.")

(defun get-archive-path (name version)
  "Get the filesystem path for a package archive."
  (when *storage-directory*
    (merge-pathnames (format nil "~A/~A/archive.tar.gz" name version)
                     *storage-directory*)))

(defun store-package-archive (name version content checksum)
  "Store a package archive on the server.
  ...
  (verify-checksum content checksum)
  (let ((path (get-archive-path name version)))
