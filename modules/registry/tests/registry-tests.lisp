...
(deftest test-compute-checksum
  ...
  (let ((checksum (reg:compute-checksum "hello world")))
    (is (str:starts-with-p checksum "sha256:"))
...
(deftest test-https-enforcement
  "Test that insecure URLs are rejected"
  (is-thrown (simple-error)
    (reg:add-registry "insecure" "http://insecure.example.com/api"))
  ;; HTTPS should work
  (reg:add-registry "secure" "https://secure.example.com/api")
  (is-not-null (reg::get-registry "secure"))
  (reg:remove-registry "secure"))

(deftest test-allow-insecure-registries
  "Test that *allow-insecure-registries* permits HTTP"
  (let ((reg::*allow-insecure-registries* t))
    (reg:add-registry "insecure-test" "http://localhost:8080/api")
    (is-not-null (reg::get-registry "insecure-test"))
    (reg:remove-registry "insecure-test")))
