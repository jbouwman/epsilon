;;;; epsilon.http.streaming - HTTP Streaming Support
;;;;
...
(defun finish-request-stream (streaming-req)
  ...
  (unless (streaming-request-finished-p streaming-req)
    ...
    ;; Read response
    (pool:read-response-from-connection (streaming-request-connection streaming-req))))
...
(defun create-streaming-response (connection response-headers)
  "Create streaming response from connection and headers. Header keys
   are looked up lowercase per the parser's normalization (RFC 7230)."
...
(defun write-sse-event (stream event)
  ...
  (dolist (line (seq:realize (str:split #\Newline (sse-event-data event))))
  ...
  (let ((event (make-sse-event))
    ...
    (when data-lines
      (setf (sse-event-data event)
            (str:join #\Newline (seq:seq (nreverse data-lines)))))
    ...
    (multiple-value-bind (scheme host port path query)
      ...
      (let ((ssl-p (string= scheme "https")))
        (pool:with-http-connection (conn http-pool host port :ssl-p ssl-p)
          ...
          ;; Send request
          (pool:send-request-on-connection conn "GET" path nil nil query)

          ;; Read response headers
          (let* ((response (pool:read-response-headers (pool:http-connection-stream conn)))
      ...
      (let* ((ssl-p (string= scheme "https"))
        ...
        (pool:with-http-connection (conn http-pool host port :ssl-p ssl-p)
          ...
          (let ((buffer (make-array (streaming-config-chunk-size config)
            ...
            ;; Read response
            (let ((response (pool:read-response-from-connection conn)))
