...
(defpackage :epsilon.http.h2.frames
  (:use :cl)
  (:import
   (epsilon.string str)
   (epsilon.io io)
...
;;;; Frame Parsing
;;;;
;;;; These functions work with epsilon.io Reader/Writer protocols.

(defun read-frame-header (reader)
  "Read a 9-byte frame header from reader.
   READER must implement the epsilon.io Reader protocol."
  (let ((header (make-array 9 :element-type '(unsigned-byte 8))))
    (io:read-exact reader header)
...
(defun read-frame (reader)
  "Read a complete HTTP/2 frame from reader.
   READER must implement the epsilon.io Reader protocol."
  (let ((frame (read-frame-header reader)))
    (when (> (http2-frame-length frame) 0)
      (let ((payload (make-array (http2-frame-length frame)
                                :element-type '(unsigned-byte 8))))
        (io:read-exact reader payload)
        (setf (http2-frame-payload frame) payload)))
    frame))

(defun write-frame-header (writer frame)
  "Write a 9-byte frame header to writer.
   WRITER must implement the epsilon.io Writer protocol."
  (let ((header (make-array 9 :element-type '(unsigned-byte 8))))
    ;; Length (24 bits)
    ...
    (io:write-all writer header)))

(defun write-frame (writer frame)
  "Write a complete HTTP/2 frame to writer.
   WRITER must implement the epsilon.io Writer protocol."
  (write-frame-header writer frame)
  (when (http2-frame-payload frame)
    (io:write-all writer (http2-frame-payload frame)))
  (io:flush writer))
...
(defun make-data-frame (stream-id data &key end-stream padded)
  ...
  (let* ((flags (logior (if end-stream +flag-end-stream+ 0)
         ...
         ;; Ensure payload is the correct type - must be (vector (unsigned-byte 8))
         (payload (etypecase data
                    ...
                    (string (let ((result (make-array (length data)
                             ...
                             result))
                    (sequence (coerce data '(vector (unsigned-byte 8)))))))
...
(defun make-headers-frame (stream-id encoded-headers &key end-stream end-headers priority padded)
  "Create a HEADERS frame. ENCODED-HEADERS should be an HPACK-encoded byte vector."
  (let* ((flags (logior (if end-stream +flag-end-stream+ 0)
                       (if end-headers +flag-end-headers+ 0)
                       (if priority +flag-priority+ 0)
                       (if padded +flag-padded+ 0)))
         (payload (etypecase encoded-headers
                    ((vector (unsigned-byte 8)) encoded-headers)
                    (string (let ((result (make-array (length encoded-headers)
                                                     :element-type '(unsigned-byte 8))))
                             (loop for i from 0 below (length encoded-headers)
                                   do (setf (aref result i) (char-code (char encoded-headers i))))
                             result))
                    (sequence (coerce encoded-headers '(vector (unsigned-byte 8)))))))
