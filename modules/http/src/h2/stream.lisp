...
(defpackage :epsilon.http.h2.stream
  (:use :cl)
  (:import
...
;;;;
;;;; These provide shorter names for the http2-stream- accessors

(defun stream-id (stream)
  "Get stream ID (alias for http2-stream-id)"
  (http2-stream-id stream))

(defun stream-state (stream)
  "Get stream state (alias for http2-stream-state)"
  (http2-stream-state stream))

(defun stream-connection (stream)
  "Get stream connection (alias for http2-stream-connection)"
  (http2-stream-connection stream))

(defun stream-flow-controller (stream)
  "Get stream flow controller (alias for http2-stream-flow-controller)"
  (http2-stream-flow-controller stream))

(defun stream-dependency (stream)
  "Get stream dependency (alias for http2-stream-dependency)"
  (http2-stream-dependency stream))

(defun stream-weight (stream)
  "Get stream weight (alias for http2-stream-weight)"
  (http2-stream-weight stream))

(defun stream-headers-buffer (stream)
  "Get stream headers buffer (alias for http2-stream-headers-buffer)"
  (http2-stream-headers-buffer stream))

(defun stream-data-buffer (stream)
  "Get stream data buffer (alias for http2-stream-data-buffer)"
  (http2-stream-data-buffer stream))

(defun stream-priority (stream)
  "Get stream priority as a plist of dependency, weight, and exclusive-p"
  (list :dependency (http2-stream-dependency stream)
        :weight (http2-stream-weight stream)
        :exclusive-p (http2-stream-exclusive-p stream)))
