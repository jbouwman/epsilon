...
(defpackage :epsilon.http.h2.error
  (:use :cl)
  (:import
  ...
  (:export
   ...
   #:http2-preface-error
   #:http2-preface-error-alpn
   #:http2-preface-error-received
   #:http2-preface-error-hint
...
(define-condition http2-preface-error (http2-connection-error)
  ((error-code :initform frames:+error-protocol-error+)
   (alpn :initarg :alpn :initform nil :reader http2-preface-error-alpn)
   (received :initarg :received :initform nil :reader http2-preface-error-received)
   (hint :initarg :hint :initform nil :reader http2-preface-error-hint))
  (:report (lambda (condition stream)
             (format stream "HTTP/2 client preface mismatch~@[ (ALPN=~A)~]~@[: got ~S~]~@[. ~A~]"
                     (http2-preface-error-alpn condition)
                     (http2-preface-error-received condition)
                     (http2-preface-error-hint condition))))
  (:documentation "Connection did not open with the HTTP/2 client preface.
Usually means the peer spoke a different protocol (e.g. HTTP/1.1) to an
HTTP/2-only endpoint, or the ALPN negotiation diverged from the server's
expectations."))
