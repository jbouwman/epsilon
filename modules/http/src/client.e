...
(defun send-request (connection method path &key headers body query)
  ...
  (let* ((request-line (format-request-line method path query))
    ...
    (if (connection-ssl-p connection)
        ...
        ;; Use regular socket stream - write directly to output stream
        (let ((out-stream (net:tcp-stream-writer (connection-socket connection))))
          (write-string request out-stream)
        ...
        (let* ((headers-text (str:join #\Linefeed (seq:from-list (reverse response-lines))))
               ...
               (body (when (and content-length (> content-length 0))
                       (let ((buffer (make-string content-length))
                             (total-read 0))
                         ;; Loop to read all data (may come in chunks with unbuffered socket)
                         ...
                         ;; Truncate to actual bytes read if needed
                         (if (< total-read content-length)
                             (subseq buffer 0 total-read)
                             buffer))))
