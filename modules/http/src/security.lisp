...
(defun get-cookie-value (request cookie-name)
  ...
  (let ((cookie-header (map:get (request:request-headers request) "cookie")))
    (when cookie-header
      ...
      ;; Split on semicolon and process each cookie
      (loop for start = 0 then (1+ end)
            for end = (position #\; cookie-header :start start)
            for cookie = (subseq cookie-header start (or end (length cookie-header)))
            for trimmed = (string-trim '(#\Space) cookie)
            for eq-pos = (position #\= trimmed)
            when (and eq-pos
                      (string= (subseq trimmed 0 eq-pos) cookie-name))
              return (subseq trimmed (1+ eq-pos))
            while end))))
...
(defun decode-basic-auth (credentials)
  ...
  (let* ((decoded (enc:base64-decode-string credentials))
