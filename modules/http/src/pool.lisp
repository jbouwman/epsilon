;;;; epsilon.http.pool - Modern HTTP Connection Pool
;;;;
...
(defun destroy-http-connection-pool (pool)
  ...
  (lock:with-lock ((http-connection-pool-lock pool))
            ...
            (if ssl-p
                (progn
                  (let ((tls-context (crypto:make-client-context)))
                    ...
                    (setf stream (crypto:make-tls-stream :connection tls-conn))))

                ;; Plain HTTP - socket is already a tcp-stream from tcp-connect
                (setf stream socket))
...
(defun get-connection (http-pool host port &key ssl-p protocol timeout)
  ...
  (let* ((key (make-connection-key
         ...
         (sub-pool (lock:with-lock ((http-connection-pool-lock http-pool))
  ...
  (multiple-value-bind (scheme host port path query)
    ...
    (let ((ssl-p (string= scheme "https")))
      (with-http-connection (conn http-pool host port
        ...
        (send-request-on-connection conn method path headers body query)

        ;; Read response
        (read-response-from-connection conn)))))
...
(defun pool-stats (http-pool)
  ...
  (lock:with-lock ((http-connection-pool-lock http-pool))
...
(defun send-request-on-connection (connection method path headers body query)
...
(defun read-response-from-connection (connection)
...
(defun read-response-headers (stream)
  "Read only HTTP response headers from stream, leaving body unread.
   Returns a plist with :status and :headers.
   Used for streaming responses where body is read separately."
  (let ((response-data (make-array 0 :element-type 'character
                                   :adjustable t :fill-pointer 0))
        (headers-complete nil)
        (header-end-marker (format nil "~C~C~C~C"
                                   #\Return #\Linefeed #\Return #\Linefeed)))
    ;; Read until we find CRLFCRLF (end of headers)
    (loop until headers-complete
          do (let ((char (read-char stream nil nil)))
               (unless char (return))
               (vector-push-extend char response-data)
               ;; Check if we've reached end of headers
               (let ((len (length response-data)))
                 (when (>= len 4)
                   (when (string= (subseq response-data (- len 4))
                                  header-end-marker)
                     (setf headers-complete t))))))

    ;; Parse status line and headers
    (let* ((header-str (coerce response-data 'string))
           (line-sep (format nil "~C~C" #\Return #\Linefeed))
           (lines (seq:realize (str:split line-sep header-str)))
           (status-line (first lines))
           (status-parts (seq:realize (str:split #\Space status-line)))
           (status-code (parse-integer (second status-parts) :junk-allowed t))
           (headers map:+empty+))

      ;; Parse header lines
      (loop for line in (rest lines)
            while (and line (not (string= line "")) (not (string= line (string #\Return))))
      ...
      (list :status status-code :headers headers))))
