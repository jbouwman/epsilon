...
(package epsilon.http.test-external-servers
...
(defparameter *run-external-tests* (sb-ext:posix-getenv "EPSILON_RUN_EXTERNAL_TESTS")
  "Whether to run external server tests (disabled by default, enable with EPSILON_RUN_EXTERNAL_TESTS=1)")
...
(defmacro with-external-test ((name) &body body)
  ...
  `(if *run-external-tests*
       (progn
         ...
         ,@body)
       (test:skip (format nil "External test ~A skipped (EPSILON_SKIP_EXTERNAL_TESTS set)" ,name))))

;;;; httpbin.org Tests

(test:deftest test-httpbin-get ()
  ...
  (with-external-test ("httpbin GET")
    (handler-case
        (let ((response (http:http-get "http://httpbin.org/get")))
          ...
          (test:is-not-null (response:response-body response))
          ;; httpbin returns JSON with request details
          (test:is (search "\"url\"" (response:response-body response))))
      (error (e)
        (test:fail (format nil "httpbin GET failed: ~A" e))))))

(test:deftest test-httpbin-post ()
    ...
    (handler-case
        (let ((response (http:http-post "http://httpbin.org/post"
          ...
          (let ((body (response:response-body response)))
            (test:is (search "\"test\"" body))
            (test:is (search "\"data\"" body))))
      (error (e)
        (test:fail (format nil "httpbin POST failed: ~A" e))))))

(test:deftest test-httpbin-put ()
  ...
  (with-external-test ("httpbin PUT")
    (handler-case
        (let ((response (http:http-put "http://httpbin.org/put"
          ...
          (test:is-equal 200 (response:response-status response))
          (test:is (search "\"key\"" (response:response-body response))))
      (error (e)
        (test:fail (format nil "httpbin PUT failed: ~A" e))))))

(test:deftest test-httpbin-delete ()
  ...
  (with-external-test ("httpbin DELETE")
    (handler-case
        (let ((response (http:http-delete "http://httpbin.org/delete")))
          (test:is-equal 200 (response:response-status response)))
      (error (e)
        (test:fail (format nil "httpbin DELETE failed: ~A" e))))))

(test:deftest test-httpbin-headers ()
  ...
  (with-external-test ("httpbin headers")
    (handler-case
        (let ((response (http:http-get "http://httpbin.org/headers"
          ...
          (test:is-equal 200 (response:response-status response))
          (let ((body (response:response-body response)))
            (test:is (search "X-Custom-Header" body))
            (test:is (search "test-value" body))
            (test:is (search "epsilon-http-client" body))))
      (error (e)
        (test:fail (format nil "httpbin headers test failed: ~A" e))))))

(test:deftest test-httpbin-status-codes ()
  ...
  (with-external-test ("httpbin status codes")
    ...
    (handler-case
      ...
      (error (e)
        (test:fail (format nil "httpbin 404 test failed: ~A" e))))
    ...
    (handler-case
      ...
      (error (e)
        (test:fail (format nil "httpbin 500 test failed: ~A" e))))))

(test:deftest test-httpbin-redirect ()
  ...
  (with-external-test ("httpbin redirects")
    (handler-case
        (let ((response (http:http-get "http://httpbin.org/redirect/2")))
          ...
          (test:is (search "\"url\"" (response:response-body response))))
      (error (e)
        (test:fail (format nil "httpbin redirect test failed: ~A" e))))))

(test:deftest test-httpbin-basic-auth ()
  "Test basic authentication"
  (with-external-test ("httpbin basic auth")
    (handler-case
        (let* ((auth-header (format nil "Basic ~A"
                                    (base64:string-to-base64-string "user:passwd")))
               (response (http:http-get "http://httpbin.org/basic-auth/user/passwd"
                                       :headers (map:make-map
                                                "Authorization" auth-header))))
          (test:is-equal 200 (response:response-status response))
          (let ((body (response:response-body response)))
            (test:is (search "\"authenticated\"" body))
            (test:is (search "true" body))))
      (error (e)
        (test:fail (format nil "httpbin basic auth test failed: ~A" e))))))

;;;; HTTPS Tests with httpbin.org

(test:deftest test-https-httpbin-get ()
  ...
  (with-external-test ("httpbin HTTPS GET")
    (handler-case
        (let ((response (http:http-get "https://httpbin.org/get")))
          ...
          (test:is-not-null (response:response-body response)))
      (error (e)
        (test:fail (format nil "httpbin HTTPS GET failed: ~A" e))))))

(test:deftest test-https-httpbin-post ()
    ...
    (handler-case
        (let ((response (http:http-post "https://httpbin.org/post"
                                        ...
                                        :headers (map:make-map
                                                 "Content-Type" "application/json"))))
          (test:is-equal 200 (response:response-status response))
          (test:is (search "\"secure\"" (response:response-body response))))
      (error (e)
        (test:fail (format nil "httpbin HTTPS POST failed: ~A" e))))))

;;;; postman-echo.com Tests

(test:deftest test-postman-echo-get ()
  ...
  (with-external-test ("postman-echo GET")
    (handler-case
        (let ((response (http:http-get "https://postman-echo.com/get?foo=bar")))
          (test:is-equal 200 (response:response-status response))
          (let ((body (response:response-body response)))
            (test:is (search "\"foo\"" body))
            (test:is (search "\"bar\"" body))))
      (error (e)
        (test:fail (format nil "postman-echo GET failed: ~A" e))))))

(test:deftest test-postman-echo-post ()
    ...
    (handler-case
        (let ((response (http:http-post "https://postman-echo.com/post"
                                        ...
                                        :headers (map:make-map
                                                 "Content-Type" "application/json"))))
          (test:is-equal 200 (response:response-status response))
          (test:is (search "\"message\"" (response:response-body response))))
      (error (e)
        (test:fail (format nil "postman-echo POST failed: ~A" e))))))

(test:deftest test-postman-echo-headers ()
  ...
  (with-external-test ("postman-echo headers")
    (handler-case
        (let ((response (http:http-get "https://postman-echo.com/headers"
          ...
          (test:is-equal 200 (response:response-status response))
          (test:is (search "x-test-header" (str:downcase (response:response-body response)))))
      (error (e)
        (test:fail (format nil "postman-echo headers test failed: ~A" e))))))

(test:deftest test-postman-echo-time ()
  "Test time endpoint"
  (with-external-test ("postman-echo time")
    (handler-case
        (let ((response (http:http-get "https://postman-echo.com/time/now")))
          ...
          (test:is-not-null (response:response-body response)))
      (error (e)
        (test:fail (format nil "postman-echo time test failed: ~A" e))))))

;;;; badssl.com Tests for TLS/SSL Compliance

(test:deftest test-badssl-valid-cert ()
  ...
  (with-external-test ("badssl valid cert")
    (handler-case
        (let ((response (http:http-get "https://badssl.com/")))
          (test:is-equal 200 (response:response-status response))
          (test:is (search "badssl.com" (response:response-body response))))
      (error (e)
        (test:fail (format nil "badssl valid cert test failed: ~A" e))))))

(test:deftest test-badssl-sha256 ()
  ...
  (with-external-test ("badssl SHA256")
    (handler-case
        (let ((response (http:http-get "https://sha256.badssl.com/")))
          (test:is-equal 200 (response:response-status response)))
      (error (e)
        (test:fail (format nil "badssl SHA256 test failed: ~A" e))))))

(test:deftest test-badssl-expired-cert ()
...
(test:deftest test-badssl-wrong-host ()
...
(test:deftest test-badssl-self-signed ()
  ...
  (with-external-test ("large response")
    (handler-case
        ...
        (let ((response (http:http-get "http://httpbin.org/bytes/1000")))
          (test:is-equal 200 (response:response-status response))
          (test:is-equal 1000 (length (response:response-body response))))
      (error (e)
        (test:fail (format nil "Large response test failed: ~A" e))))))

(test:deftest test-streaming-response ()
    ...
    (handler-case
        ...
        (let ((response (http:http-get "http://httpbin.org/stream/10")))
          ...
          (let ((lines (str:split #\Newline (response:response-body response))))
            (test:is-equal 10 (length (remove-if #'str:empty-p lines)))))
      (error (e)
        (test:fail (format nil "Streaming response test failed: ~A" e))))))

(test:deftest test-gzip-response ()
  ...
  (with-external-test ("gzip response")
    (handler-case
        (let ((response (http:http-get "http://httpbin.org/gzip")))
          ...
          (test:is (search "\"gzipped\"" (response:response-body response))))
      (error (e)
        (test:fail (format nil "Gzip response test failed: ~A" e))))))

(test:deftest test-concurrent-requests ()
...
(test:deftest test-external-servers-summary ()
