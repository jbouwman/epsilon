...
(package epsilon.http.test-runner
  (import (epsilon.string str)
          (epsilon.test.suite test-suite))
  ...
  (let ((start-time (get-internal-real-time))
    ...
    (handler-case
        (progn
          ...
          (when skip-external
            (sb-posix:setenv "EPSILON_SKIP_EXTERNAL_TESTS" "1" 1))
        ...
        (let ((times nil))
          (dotimes (i 10)
            (let ((req-start (get-internal-real-time)))
              (epsilon.http:http-get "http://httpbin.org/get")
                     ...
                     (handler-case
                         (progn
                           ...
                           (sb-thread:with-mutex (lock)
                             (incf (car success-count))))
                       (error () nil))))
                  threads))
          (dolist (thread threads)
            (sb-thread:join-thread thread))
          (format t "   Successful requests: ~D/20~%" (car success-count)))
      (error (e)
        (format t "   Error: ~A~%" e)))
...
(defun test-report ()
  ...
  (when *test-results*
    (let* ((total (length *test-results*))
      ...
      (cond
        ((= failed 0)
         (format t "[PASS] All tests passed!~%"))
        ((< failed 3)
         (format t "[WARN] Minor issues detected (~D failures)~%" failed))
        (t
         (format t "[FAIL] Multiple failures detected (~D failures)~%" failed))))))
...
(defun run-all-tests (&key skip-external skip-performance)
  ...
  (format t "+=====================================================+~%")
  (format t "~%Starting HTTP module testing...~%")
