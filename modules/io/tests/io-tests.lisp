...
(deftest test-async-read-creation
  ...
  (with-io-context (ctx)
    (let ((buf (allocate-buffer 1024)))
      ;; Use a mock fd that won't cause epoll errors
      (let ((op (async-read 999 buf :context ctx)))
        (is (epsilon.async:async-operation-p op))
        (is-= 999 (epsilon.async:async-operation-fd op))
...
(deftest test-async-write-creation
  ...
  (with-io-context (ctx)
    (let ((buf (string-to-buffer "test data")))
      ;; Use a mock fd that won't cause epoll errors
      (let ((op (async-write 998 buf :context ctx)))
        (is (epsilon.async:async-operation-p op))
        (is-= 998 (epsilon.async:async-operation-fd op))
