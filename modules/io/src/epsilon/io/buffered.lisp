...
(defpackage epsilon.io.buffered
  (:use :cl)
...
(defun buffered-reader-fill (reader)
  ...
  (let* ((buf (buffered-reader-buffer reader))
    ...
    ;; Fill rest of buffer
    (let ((n (proto:read-into (buffered-reader-source reader) buf
                        :start remaining
                        :end (length buf))))
      (when (zerop n)
...
(tc:definstance proto:closer buffered-reader
  (proto:close* (reader)
...
(tc:definstance proto:reader buffered-reader
  ...
  (proto:read-byte* (reader)
    ...
    (when (= (buffered-reader-position reader) (buffered-reader-limit reader))
      (when (zerop (buffered-reader-fill reader))
        (return-from proto:read-byte* nil)))
...
(tc:definstance proto:closer buffered-writer
  (proto:close* (writer)
...
(tc:definstance proto:writer buffered-writer
...
(defun buffered-writer-write-line (writer string &key (encoding :utf-8))
  ...
  (proto:write-byte* writer (char-code #\Newline)))
