...
(defpackage epsilon.io.byte-stream
  (:use :cl)
...
(tc:definstance proto:closer byte-reader
  (proto:close* (reader)
...
(tc:definstance proto:reader byte-reader
...
(tc:definstance proto:seeker byte-reader
  (proto:seek* (reader offset whence)
...
(tc:definstance proto:closer byte-writer
  (proto:close* (writer)
...
(tc:definstance proto:writer byte-writer
...
;;; byte-writer supports position*/size* but not seek*, so these remain standalone
...
(tc:definstance proto:closer null-reader
  (proto:close* (reader)
...
(tc:definstance proto:reader null-reader
  (proto:read-into (reader buffer &key start end)
    (declare (ignore reader buffer start end))
    0)
  (proto:read-byte* (reader)
    (declare (ignore reader))
    nil))
...
(tc:definstance proto:closer null-writer
  (proto:close* (writer)
...
(tc:definstance proto:writer null-writer
  (proto:write-from (writer buffer &key (start 0) (end (length buffer)))
    (declare (ignore writer))
    (- end start))
  (proto:write-byte* (writer byte)
    (declare (ignore writer byte))
    1)
  (proto:flush (writer)
    (declare (ignore writer))
    nil))
