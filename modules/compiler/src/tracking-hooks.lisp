...
(defpackage epsilon.compile-hooks
  ...
  (:import (epsilon.compile-api api)
           ...
           (epsilon.map map)
           (epsilon.string str))
  (:export #:*capture-compilation-output*
...
(defvar *macro-expansion-counter*
  0
      ...
      (when (typep context 'sb-c::compiler-error-context)
        (let ((file-name (sb-c::compiler-error-context-file-name context))
          ...
          (when (and file-name (not (eq file-name :lisp)))
            (setf file (if (pathnamep file-name) (namestring file-name) file-name)))
          ;; Try to convert file position to line number
          (when (and file file-position) (setf line (estimate-line-number file file-position)))))
...
(defun estimate-line-number (file position)
  ...
  (when (and file position (probe-file file))
    (handler-case (with-open-file (stream file :direction :input)
      (let ((line-count 1) (char-count 0))
        (loop
          for
          char
          =
          ...
          while
          (and char (< char-count position))
          do
          (progn (incf char-count) (when (char= char #\Newline) (incf line-count))))
        line-count))
      (error () nil))))
...
(defun extract-source-location ()
  ...
  (cond
    ...
    ((and (boundp 'sb-c::*compiler-error-context*) sb-c::*compiler-error-context*) (extract-source-location-from-context sb-c::*compiler-error-context*))
    ;; Fall back to basic file information
    ((and (boundp 'sb-c::*compile-file-pathname*) sb-c::*compile-file-pathname*)
     (loc:make-source-location :file (namestring sb-c::*compile-file-pathname*)))
...
(defun extract-compiler-context ()
  ...
  (let ((context nil) (source-path nil) (enclosing-forms nil))
...
(defun capture-compiler-condition (condition severity)
  "Capture a compiler condition as a structured diagnostic."
  (when *capture-compilation-output*
    (let ((location (extract-source-location)))
      (multiple-value-bind (context source-path enclosing-forms) (extract-compiler-context)
        (declare (ignore source-path))
        (let ((d (diag:make-diagnostic :severity severity
                                       :message (princ-to-string condition)
                                       :location location
                                       :notes (when context (list (princ-to-string context)))
                                       :enclosing-form enclosing-forms)))
          (push d *current-compilation-messages*))))))
...
(defun warning-hook-handler (condition)
  ...
  (when *capture-compilation-output* (muffle-warning condition)))
...
(defun update-compilation-statistics ()
  ...
  (when *current-compilation-statistics*
    (incf (cr:compilation-statistics-forms-processed *current-compilation-statistics*))))

(defun track-function-compilation ()
  "Track that a function was compiled."
  (when *current-compilation-statistics*
    (incf (cr:compilation-statistics-functions-compiled *current-compilation-statistics*))))

(defun track-macro-expansion ()
  "Track that a macro was expanded."
  (when *current-compilation-statistics*
    (incf (cr:compilation-statistics-macros-expanded *current-compilation-statistics*))))
...
(defmacro with-compilation-capture (() &body body)
  ...
  (let ((body-result (gensym "BODY-RESULT-")))
    `(let ((*capture-compilation-output* t) (*current-compilation-messages* nil)
                                            (*current-compilation-statistics* (cr:make-compilation-statistics))
                                            (*form-counter* 0)
                                            (*function-counter* 0)
                                            (*macro-expansion-counter* 0))
      (handler-bind ((sb-c::compiler-error #'compiler-error-hook-handler) (warning #'warning-hook-handler)
                                                                          (style-warning #'style-warning-hook-handler))
        (let ((,body-result (progn ,@body)))
          ;; Update final statistics
          (when *current-compilation-statistics*
            (setf (cr:compilation-statistics-forms-processed *current-compilation-statistics*)
                  *form-counter*)
            (setf (cr:compilation-statistics-functions-compiled *current-compilation-statistics*)
                  *function-counter*)
            (setf (cr:compilation-statistics-macros-expanded *current-compilation-statistics*)
