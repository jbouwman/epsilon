...
(defun get-file-modification-time (pathname)
  ...
  (handler-case (file-write-date pathname) (error () nil)))

(defun cache-file-lines (pathname)
  "Cache line positions for a file."
  (let ((truename (truename pathname)))
    (handler-case (let ((mod-time (get-file-modification-time truename)) (line-starts (make-array 1000
                                                                                                  :adjustable
                                                                                                  t
                                                                                                  :fill-pointer
                                                                                                  0)))
      ...
      (with-open-file (stream truename :direction :input)
        (let ((pos 0))
          (loop
            ...
            char
            do
            (progn (incf pos) (when (char= char #\Newline) (vector-push-extend pos line-starts))))))
      ;; Create and cache the result
      (let ((cache (make-file-line-cache :pathname
                                         truename
                                         :line-starts
                                         (coerce line-starts 'simple-vector)
                                         :modification-time
                                         mod-time)))
...
(defun get-cached-file-lines (pathname)
  ...
  (let* ((truename (handler-case (truename pathname)
                     (error () (return-from get-cached-file-lines nil)))) (cached (gethash truename
      ...
      (when cache
        (let ((line-starts (file-line-cache-line-starts cache)))
          ...
          (let ((line-num (position-if (lambda (start)
                                       ...
                                       line-starts)))
            (if line-num
              (values line-num (1+ (- position (aref line-starts (1- line-num)))))
...
(defun extract-sbcl-source-location ()
  ...
  (let ((file nil) (position nil) (toplevel-form nil) (form-number nil))
    ...
    (when (and (boundp 'sb-c::*compiler-error-context*) sb-c::*compiler-error-context*)
      (typecase sb-c::*compiler-error-context*
        (sb-c::compiler-error-context (let ((context sb-c::*compiler-error-context*))
                                        (setf position
                                              (sb-c::compiler-error-context-file-position context))
                                        (setf toplevel-form
                                              (sb-c::compiler-error-context-original-form context))))))
    ;; Try to get form number from current path
    (when (and (boundp 'sb-c::*current-path*) sb-c::*current-path*)
      (let ((path sb-c::*current-path*)) (when (consp path) (setf form-number (length path)))))
...
(defun get-current-source-location ()
  ...
  (multiple-value-bind (file position toplevel-form form-number) (extract-sbcl-source-location)
    (declare (ignore toplevel-form form-number))
    (multiple-value-bind (line column) (when (and file position)
      (file-position-to-line-column file position))
      (loc:make-source-location :file (when file (namestring file))
                                :line line
                                :column column))))

(defun enhance-location-with-line-info (location)
  "Enhance a source location with line/column information if missing."
  (if (or (loc:source-location-line location) (not (loc:source-location-file location)))
    location
    ;; Try to extract line info from current compiler state
    (let ((current (get-current-source-location)))
      (if (and current
               (equal (loc:source-location-file current) (loc:source-location-file location)))
        (loc:make-source-location :file (loc:source-location-file location)
                                  :line (loc:source-location-line current)
                                  :column (loc:source-location-column current))
        location))))

;;; Source tracking context manager
(defvar *source-tracking-enabled*
  nil
  "When true, source location tracking is active.")

(defvar *current-tracked-location*
  nil
...
(defmacro with-source-tracking ((&key (enable t)) &body body)
  ...
  `(let ((*source-tracking-enabled* ,enable) (*current-tracked-location* nil)) ,@body))

(defun track-source-location (form)
  "Track the source location of a form being compiled."
  (declare (ignore form))
  (when *source-tracking-enabled* (setf *current-tracked-location* (get-current-source-location))))
...
(defun source-location-string (location)
  ...
  (when location
    (format nil
            "~@[~A~]~@[:~A~]"
            (loc:source-location-file location)
            (when (and (loc:source-location-line location) (loc:source-location-column location))
              (format-line-column (loc:source-location-line location)
                                  (loc:source-location-column location))))))
