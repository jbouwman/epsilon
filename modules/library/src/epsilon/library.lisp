  ...
  (setf *search-paths*
        (append
         ...
         (when-let ((epsilon-path (sb-ext:posix-getenv "EPSILON_LIBRARY_PATH")))
           (seq:realize (str:split #\: epsilon-path)))
...
(defun get-bundled-library-root ()
  "Get root directory for bundled libraries (epsilon/vendor/lib/)"
  (or *bundled-library-root*
      (setf *bundled-library-root*
            (let ((epsilon-home (sb-ext:posix-getenv "EPSILON_HOME")))
              (if epsilon-home
                  (path:string-path-join epsilon-home "vendor" "lib")
...
(defun register-bundled-library (base-name source-path)
  ...
  (declare (ignore base-name))
  (let* ((platform-dir (path:string-path-join (get-bundled-library-root)
                                              (platform-string)))
         (lib-name (fs:strip-extension (fs:basename source-path)))
  ...
  (let ((env-paths (sb-ext:posix-getenv "EPSILON_LIBRARY_PATH")))
    (when env-paths
      (find-library-in-paths (library-info-search-names info)
                            (seq:realize (str:split #\: env-paths))))))
  ...
  (handler-case
      (let ((path (find-library name)))
        (cond
          ...
          ;; function pointers (in *function-cache*) would then point to
          ;; unmapped memory, causing SIGSEGV on the next FFI call.
          (path
           (handler-case
...
(defun available-libraries ()
  ...
  (let ((available nil))
    (loop for (name . info) in (map:to-alist *library-registry*)
...
(defun lib-close (library-handle)
  ...
  (if (null library-handle)
      ...
      (loop for (name . handle) in (map:to-alist *open-libraries*)
...
(eval-when (:load-toplevel :execute)
  ...
  ;; Generic names only - the dynamic linker finds them via LD_LIBRARY_PATH
  ;; (set by flake.nix shellHook) or standard system paths.
  ;; Do NOT hardcode /nix/store/ hashes or Homebrew version paths here.
  ...
  ;; SQLite - register all common name variants
