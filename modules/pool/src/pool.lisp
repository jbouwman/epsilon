...
(defstruct pool
  ...
  (lock (lock:make-lock "pool") :type lock:lock)
  (condition (cv:make-condition-variable :name "pool-notify") :type cv:condition-variable)
  (maintenance-thread nil :type (or null thread:thread))
...
(defun destroy-pool (pool)
  ...
  (let ((maintenance-thread nil))
    (lock:with-lock ((pool-lock pool))
      ...
      (cv:condition-broadcast (pool-condition pool)))

    ;; Shutdown maintenance thread outside of mutex
    (when maintenance-thread
      (thread:join-thread maintenance-thread))))
    ...
    (loop
      (lock:with-lock ((pool-lock pool))
        ...
        (let ((remaining-time (when timeout-time
          ...
          (cv:condition-wait (pool-condition pool) (pool-lock pool)
  ...
  (lock:with-lock ((pool-lock pool))
    ...
    (if (and (pool-config-validation-on-release (pool-config pool))
        ...
        (let ((wrapped (make-pooled-resource
          ...
          ;; Signal waiting threads
          (cv:condition-notify (pool-condition pool))))))
...
(defun map-resources (pool fn)
  "Call FN on each resource in POOL (both available and in-use).
   Called under the pool lock for consistency."
  (lock:with-lock ((pool-lock pool))
    (dolist (wrapped (pool-available pool))
      (funcall fn (pooled-resource-resource wrapped)))
    (maphash (lambda (resource time)
               (declare (ignore time))
               (funcall fn resource))
             (pool-in-use pool))))
