...
;;;;  looking-at-p) could become epsilon.lexer.

(package epsilon.regex
  (use epsilon.syntax epsilon.symbol epsilon.type)
  (shadow compile search)
  (import (epsilon.charset charset)
          (epsilon.string str)))
...
we don't tolerate whitespace in front of the number."
  (when (or (end-of-string-p lexer)
            (and no-whitespace-p
                 (str:whitespacep (schar (lexer-str lexer) (lexer-pos lexer)))))
                ...
                collect (cond ((characterp item)
                              ...
                              ((symbolp item)
                               (case item
                                 ...
                                 ((:whitespace-char-class) #'str:whitespacep)
                                 ((:non-whitespace-char-class) (charset:complement* #'str:whitespacep))
                                 ((:word-char-class) #'str:word-char-p)
                                 ((:non-word-char-class) (charset:complement* #'str:word-char-p))
                                 (otherwise
...
parse trees which are atoms.")
  ...
  (:method ((parse-tree (eql :word-char-class)))
    (declare (special accumulate-start-p))
    (setq accumulate-start-p nil)
    (make-instance 'char-class :test-function #'str:word-char-p))
  (:method ((parse-tree (eql :whitespace-char-class)))
    (declare (special accumulate-start-p))
    (setq accumulate-start-p nil)
    (make-instance 'char-class :test-function #'str:whitespacep))
  ...
  (:method ((parse-tree (eql :non-word-char-class)))
    (declare (special accumulate-start-p))
    (setq accumulate-start-p nil)
    (make-instance 'char-class :test-function (charset:complement* #'str:word-char-p)))
  (:method ((parse-tree (eql :non-whitespace-char-class)))
    (declare (special accumulate-start-p))
    (setq accumulate-start-p nil)
    (make-instance 'char-class :test-function (charset:complement* #'str:whitespacep)))
...
(defun word-boundary-p (start-pos)
  ...
  (let ((1-start-pos (1- start-pos))
    ...
    (or (and (or (= start-pos *end-pos*)
                 (and (< start-pos *end-pos*)
                      (not (str:word-char-p (schar *string* start-pos)))))
             (and (< 1-start-pos *end-pos*)
                  (<= *start-pos* 1-start-pos)
                  (str:word-char-p (schar *string* 1-start-pos))))
        (and (or (= start-pos *start-pos*)
                 (and (< 1-start-pos *end-pos*)
                      (<= *start-pos* 1-start-pos)
                      (not (str:word-char-p (schar *string* 1-start-pos)))))
             (and (< start-pos *end-pos*)
                  (str:word-char-p (schar *string* start-pos)))))))
...
share structure with TARGET-STRING."
  (multiple-value-bind (match-start match-end reg-starts reg-ends)
    ...
    (let ((substr-fn (if sharedp #'str:nsubseq #'subseq)))
...
structure with TARGET-STRING."
  (let ((pos-list (list start))
    ...
    (push end pos-list)
    (nreverse
     (loop with substr-fn = (if sharedp #'str:nsubseq #'subseq)
            ...
            do (typecase token
                 ...
                 (function
                  (write-string
                   (cond (simple-calls
                          (apply token
                                 (str:nsubseq target-string match-start match-end)
                                 (map 'list
                                      (lambda (reg-start reg-end)
                                        (and reg-start
                                             (str:nsubseq target-string reg-start reg-end)))
                                      reg-starts reg-ends)))
                         (t
                          (funcall token
                                   target-string
...
#\\Newline."
    (flet ((remove-tokens (target-string start end match-start
             ...
             (loop for result = (str:nsubseq target-string match-start match-end)
