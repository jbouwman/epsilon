(defpackage :epsilon.url.tests
  (:use :cl :epsilon.test :epsilon.url))

(in-package :epsilon.url.tests)
...
(deftest test-url-to-path
  ...
  (let ((path (url-to-path "file:///usr/local/bin/test")))
    (is (typep path 'epsilon.path:path))
    (is-equal "/usr/local/bin/test" (epsilon.path:path-string path))))
...
(deftest test-path-url-roundtrip
  ...
  (let* ((original-path "/usr/local/bin/test")
    ...
    (is-equal original-path (epsilon.path:path-string path))))
