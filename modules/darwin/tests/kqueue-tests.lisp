...
(deftest basic-kqueue-creation
  ...
  (let ((kq (kqueue:kqueue)))
    (assert-true (numberp kq) "kqueue should return a file descriptor")
    (assert-true (>= kq 0) "kqueue file descriptor should be non-negative")
...
(deftest with-kqueue-macro
  ...
  (let (fd-captured)
    ...
    (assert-true (numberp fd-captured) "File descriptor should have been captured")))

(deftest event-constants
  "Test that event constants are defined correctly"
  (assert-true (= kqueue:+evfilt-read+ -1) "EVFILT_READ should be -1")
  (assert-true (= kqueue:+evfilt-write+ -2) "EVFILT_WRITE should be -2")
  (assert-true (= kqueue:+ev-add+ #x0001) "EV_ADD should be 0x0001")
  (assert-true (= kqueue:+ev-delete+ #x0002) "EV_DELETE should be 0x0002"))
...
(deftest kevent-struct-creation
  ...
  (let ((event (kqueue:make-kevent-struct
                ...
                :data 100
                :udata 200)))
    (assert-true (= (kqueue:kevent-struct-ident event) 42) "ident should be 42")
    (assert-true (= (kqueue:kevent-struct-filter event) kqueue:+evfilt-read+) "filter should be EVFILT_READ")
    (assert-true (= (kqueue:kevent-struct-flags event) kqueue:+ev-add+) "flags should be EV_ADD")
    (assert-true (= (kqueue:kevent-struct-data event) 100) "data should be 100")
    (assert-true (= (kqueue:kevent-struct-udata event) 200) "udata should be 200")))
...
(deftest timespec-struct-creation
  ...
  (let ((ts (kqueue:make-timespec-struct :tv-sec 10 :tv-nsec 500000000)))
    (assert-true (= (kqueue:timespec-struct-tv-sec ts) 10) "tv_sec should be 10")
    (assert-true (= (kqueue:timespec-struct-tv-nsec ts) 500000000) "tv_nsec should be 500000000")))
  ...
  (kqueue:with-kqueue (kq)
    ...
    (handler-case
        (progn
          ...
          (assert-true t "Event add/remove cycle completed without crashing"))
      (error (e)
        ;; If it fails (e.g., stdin not suitable), that's still a valid test result
        (assert-true t (format nil "Event operations failed as expected: ~A" e))))))
...
(deftest poll-events-timeout
  ...
  (kqueue:with-kqueue (kq)
    ...
    (let ((events (kqueue:poll-events kq :max-events 1 :timeout 0)))
      (assert-true (listp events) "poll-events should return a list")
      (assert-true (= (length events) 0) "Should get no events with immediate timeout"))))
...
(deftest test-make-kevent-memory
  ...
  ;; Test empty list
  (assert-true (zerop (second (multiple-value-list (kqueue::make-kevent-memory '()))))
  ...
  (let* ((kevent (kqueue:make-kevent-struct
    ...
    (multiple-value-bind (memory count)
      ...
      (unwind-protect
           (progn
             (assert-true (= count 1) "Should have count of 1")
             (assert-true (not (zerop (sb-sys:sap-int memory))) "Memory should be allocated")
             ;; Verify the structure was written correctly
             (assert-true (= 123 (sb-sys:sap-ref-64 memory 0)) "ident should be 123")
             (assert-true (= kqueue:+evfilt-read+ (sb-sys:signed-sap-ref-16 memory 8)) "filter should be EVFILT_READ")
             (assert-true (= kqueue:+ev-add+ (sb-sys:sap-ref-16 memory 10)) "flags should be EV_ADD")
             (assert-true (= 0 (sb-sys:sap-ref-32 memory 12)) "fflags should be 0")
             (assert-true (= 456 (sb-sys:sap-ref-64 memory 16)) "data should be 456")
             (assert-true (= 789 (sb-sys:sap-ref-64 memory 24)) "udata should be 789"))
  ...
  (let* ((kevent1 (kqueue:make-kevent-struct :ident 1 :filter -1 :flags 1 :fflags 0 :data 10 :udata 20))
    ...
    (multiple-value-bind (memory count)
      ...
      (unwind-protect
           (progn
             (assert-true (= count 2) "Should have count of 2")
             ;; Check first kevent
             (assert-true (= 1 (sb-sys:sap-ref-64 memory 0)) "First ident should be 1")
             ;; Check second kevent (offset by 32 bytes)
             (assert-true (= 2 (sb-sys:sap-ref-64 memory 32)) "Second ident should be 2"))
        (when memory (lib:foreign-free memory)))))
  ...
  (let* ((kevent (kqueue:make-kevent-struct :ident nil :filter nil :flags nil))
    ...
    (multiple-value-bind (memory count)
      ...
      (unwind-protect
           (progn
             (assert-true (= count 1) "Should handle nil values")
             (assert-true (= 0 (sb-sys:sap-ref-64 memory 0)) "nil ident should be 0")
             (assert-true (= 0 (sb-sys:signed-sap-ref-16 memory 8)) "nil filter should be 0")
             (assert-true (= 0 (sb-sys:sap-ref-16 memory 10)) "nil flags should be 0"))
        (when memory (lib:foreign-free memory))))))
    ...
    (unwind-protect
         (progn
           ...
           (let ((kevents (kqueue::parse-kevent-memory memory 2)))
             (assert-true (= 2 (length kevents)) "Should parse 2 kevents")

             ;; Check first kevent
             (let ((ke1 (first kevents)))
               (assert-true (= 100 (kqueue:kevent-struct-ident ke1)) "First ident")
               (assert-true (= -1 (kqueue:kevent-struct-filter ke1)) "First filter")
               (assert-true (= 1 (kqueue:kevent-struct-flags ke1)) "First flags")
               (assert-true (= 2 (kqueue:kevent-struct-fflags ke1)) "First fflags")
               (assert-true (= 200 (kqueue:kevent-struct-data ke1)) "First data")
               (assert-true (= 300 (kqueue:kevent-struct-udata ke1)) "First udata"))

             ;; Check second kevent
             (let ((ke2 (second kevents)))
               (assert-true (= 400 (kqueue:kevent-struct-ident ke2)) "Second ident")
               (assert-true (= -2 (kqueue:kevent-struct-filter ke2)) "Second filter")
               (assert-true (= 3 (kqueue:kevent-struct-flags ke2)) "Second flags")
               (assert-true (= 4 (kqueue:kevent-struct-fflags ke2)) "Second fflags")
               (assert-true (= 500 (kqueue:kevent-struct-data ke2)) "Second data")
               (assert-true (= 600 (kqueue:kevent-struct-udata ke2)) "Second udata"))))
...
(deftest test-make-timespec-memory
  ...
  (let ((memory (kqueue::make-timespec-memory 0 0)))
    (unwind-protect
         (progn
           (assert-true (= 0 (sb-sys:sap-ref-64 memory 0)) "Zero seconds")
           (assert-true (= 0 (sb-sys:sap-ref-64 memory 8)) "Zero nanoseconds"))
      (lib:foreign-free memory)))
  ...
  (let ((memory (kqueue::make-timespec-memory 10 500000000)))
    (unwind-protect
         (progn
           (assert-true (= 10 (sb-sys:sap-ref-64 memory 0)) "10 seconds")
           (assert-true (= 500000000 (sb-sys:sap-ref-64 memory 8)) "500000000 nanoseconds"))
      (lib:foreign-free memory)))
  ...
  (let ((memory (kqueue::make-timespec-memory 9999999 999999999)))
    (unwind-protect
         (progn
           (assert-true (= 9999999 (sb-sys:sap-ref-64 memory 0)) "Large seconds")
           (assert-true (= 999999999 (sb-sys:sap-ref-64 memory 8)) "Max nanoseconds"))
      (lib:foreign-free memory))))
...
(deftest test-all-event-constants
  ...
  ;; Event filters
  (assert-true (= kqueue:+evfilt-read+ -1) "EVFILT_READ")
  (assert-true (= kqueue:+evfilt-write+ -2) "EVFILT_WRITE")
  (assert-true (= kqueue:+evfilt-except+ -15) "EVFILT_EXCEPT")
  (assert-true (= kqueue:+evfilt-signal+ -6) "EVFILT_SIGNAL")
  (assert-true (= kqueue:+evfilt-timer+ -7) "EVFILT_TIMER")
  (assert-true (= kqueue:+evfilt-vnode+ -4) "EVFILT_VNODE")
  (assert-true (= kqueue:+evfilt-proc+ -5) "EVFILT_PROC")
  (assert-true (= kqueue:+evfilt-user+ -10) "EVFILT_USER")

  ;; Event flags
  (assert-true (= kqueue:+ev-add+ #x0001) "EV_ADD")
  (assert-true (= kqueue:+ev-delete+ #x0002) "EV_DELETE")
  (assert-true (= kqueue:+ev-enable+ #x0004) "EV_ENABLE")
  (assert-true (= kqueue:+ev-disable+ #x0008) "EV_DISABLE")
  (assert-true (= kqueue:+ev-oneshot+ #x0010) "EV_ONESHOT")
  (assert-true (= kqueue:+ev-clear+ #x0020) "EV_CLEAR")
  (assert-true (= kqueue:+ev-eof+ #x8000) "EV_EOF")
  (assert-true (= kqueue:+ev-error+ #x4000) "EV_ERROR"))
      ...
      (unwind-protect
           (progn
             ...
             (let ((events (kqueue:poll-events kq :timeout 0)))
               (assert-true (null events) "No events initially"))
             ...
             (let ((events (kqueue:poll-events kq :timeout 0.1)))
               (assert-true (= 1 (length events)) "Should get one event")
               (when events
                 (let ((event (first events)))
                   (assert-true (= read-fd (kqueue:kevent-struct-ident event)) "Event for correct fd")
                   (assert-true (= kqueue:+evfilt-read+ (kqueue:kevent-struct-filter event)) "Read filter"))))
             ...
             (let ((events (kqueue:poll-events kq :timeout 0)))
               (assert-true (null events) "No events after removal")))
        ...
        (unwind-protect
             (progn
               ...
               (let ((events (kqueue:poll-events kq :max-events 10 :timeout 0.1)))
                 (assert-true (= 2 (length events)) "Should get two events")

                 ;; Check both fds are represented
                 (let ((fds (mapcar #'kqueue:kevent-struct-ident events)))
                   (assert-true (member read-fd1 fds) "First pipe should have event")
                   (assert-true (member read-fd2 fds) "Second pipe should have event"))))
    ...
    (multiple-value-bind (read-fd write-fd) (create-pipe)
      (unwind-protect
           (progn
             ...
             (let ((events (kqueue:poll-events kq :timeout 0)))
               (assert-true (null events) "Oneshot event should be consumed"))
             ...
             (let ((events (kqueue:poll-events kq :timeout 0.1)))
               (assert-true (not (null events)) "EV_CLEAR event should trigger")))
        (close-pipe read-fd write-fd)))))
  ...
  (kqueue:with-kqueue (kq)
    ...
    (let ((start-time (get-internal-real-time)))
      (let ((events (kqueue:wait-for-events kq 10 0)))
        (assert-true (null events) "Immediate timeout should return empty")
      ...
      (unwind-protect
           (progn
             ...
             (let ((events (kqueue:wait-for-events kq 1 nil)))
               (assert-true (= 1 (length events)) "Should get event with nil timeout")))
        (close-pipe read-fd write-fd)))))
...
(deftest test-kevent-error-conditions
  ...
  (handler-case
      (progn
        (kqueue:kevent -1 '() '() 0)
        (assert-true nil "Should have thrown an error for invalid kqueue"))
    (error (e)
      (assert-true t (format nil "Got expected error: ~A" e))))
  ...
  (let ((kq (kqueue:kqueue)))
    ...
    (handler-case
        (progn
          (kqueue:add-event kq 0 kqueue:+evfilt-read+)
          (assert-true nil "Should have thrown an error for closed kqueue"))
      (error (e)
        (assert-true t (format nil "Got expected error: ~A" e))))))
  ...
  (handler-case
      (progn
        (with-output-to-string (*standard-output*)
          (kqueue::test-kqueue))
        (assert-true t "test-kqueue completed without error"))
    (error (e)
      (assert-true t (format nil "test-kqueue raised expected error: ~A" e)))))
