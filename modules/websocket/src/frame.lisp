...
(defstruct (websocket-frame (:constructor %make-websocket-frame))
  ...
  (payload (make-array 0 :element-type '(unsigned-byte 8)) :type (simple-array (unsigned-byte 8) (*))))       ; Frame payload

(defun make-websocket-frame (&key (fin t) (rsv1 nil) (rsv2 nil) (rsv3 nil) 
                                  (opcode 0) (masked nil) mask payload)
  "Create a WebSocket frame, ensuring payload is proper byte array"
  (let ((coerced-payload 
         (cond
           ((null payload)
            (make-array 0 :element-type '(unsigned-byte 8)))
           ((typep payload '(simple-array (unsigned-byte 8) (*)))
            payload)
           ((vectorp payload)
           ...
           (t
            (error "Invalid payload type: ~A" (type-of payload)))))
        (coerced-mask
         (when mask
           (cond
             ((typep mask '(simple-array (unsigned-byte 8) (4)))
              mask)
             ((and (vectorp mask) (= (length mask) 4))
              (make-array 4
                         :element-type '(unsigned-byte 8)
                         :initial-contents mask))
             (t
              (error "Invalid mask: must be 4-byte array"))))))
    (%make-websocket-frame :fin fin :rsv1 rsv1 :rsv2 rsv2 :rsv3 rsv3
                          :opcode opcode :masked masked 
                          :mask coerced-mask :payload coerced-payload)))

(defun make-frame (&key (fin t) (opcode +opcode-text+) (masked nil) mask payload
                        (rsv1 nil) (rsv2 nil) (rsv3 nil))
...
(defun serialize-frame (frame &key (client-side nil))
  ...
  (let* ((payload (websocket-frame-payload frame))
    ...
    (bin:with-output-to-octets (stream)
      ...
      (cond
        ...
        ((< payload-len 65536)
         ...
         (let ((len-bytes (bin:uint16-to-octets payload-len :big-endian)))
           (bin:write-bytes stream len-bytes)))
        (t
      ...
      ;; Masking key
      (when needs-masking
        (bin:write-bytes stream mask))
      
      ;; Payload
      (bin:write-bytes stream masked-payload))))
...
(defun make-close-frame (code &optional reason)
  ...
  (let ((payload (bin:with-output-to-octets (stream)
                   (let ((code-bytes (bin:uint16-to-octets code :big-endian)))
                     (bin:write-bytes stream code-bytes))
                   (when reason
                     (let ((reason-bytes (str:string-to-octets reason)))
                       (bin:write-bytes stream reason-bytes))))))
    (make-websocket-frame
     :fin t
     :opcode +opcode-close+
     :payload payload)))
...
(defun parse-close-frame (frame)
  ...
  (let ((payload (websocket-frame-payload frame)))
    (if (zerop (length payload))
        ...
        (let ((code (bin:octets-to-uint16 (subseq payload 0 2) 0 :big-endian))
              (reason (when (> (length payload) 2)
                        (str:octets-to-string (subseq payload 2)))))
          (values code reason)))))

;;; Frame decoding from octets

(defun decode-frame (octets)
  "Decode a WebSocket frame from byte array"
  (let ((stream (stream:make-input-stream octets)))
    (parse-frame stream)))

;;; Close frame payload encoding

(defun encode-close-payload (code &optional reason)
  "Encode close frame payload with status code and optional reason"
  (bin:with-output-to-octets (stream)
    (let ((code-bytes (bin:uint16-to-octets code :big-endian)))
      (bin:write-bytes stream code-bytes))
    (when reason
...
(defun valid-utf8-p (octets)
  "Check if byte sequence is valid UTF-8"
  (handler-case
      ;; Try to convert the octets to a string using SBCL's UTF-8 decoder
      ;; If it succeeds without error, it's valid UTF-8
      (progn
        (sb-ext:octets-to-string octets :external-format :utf-8)
        t)
    ;; If there's any error during conversion, it's not valid UTF-8
    (error ()
      nil)))
