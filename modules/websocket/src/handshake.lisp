...
(defun valid-handshake-response-p (accept-key request-key)
  "Check if the accept key is valid for the given request key"
  (string= accept-key (generate-accept-key request-key)))
...
(defun generate-handshake-headers (host path)
  "Generate WebSocket handshake headers for a request"
  (let ((websocket-key (generate-websocket-key)))
    (map:from-pairs (list (cons "Host" host)
