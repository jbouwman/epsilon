;;;; Immutable record types with efficient field access, protocol implementation
;;;; support, and functional update semantics.
...
;; generate-deriving-instances is now shared via epsilon.typeclass.std
;; See tc-std:generate-deriving-instances
...
(defmacro defrecord (name fields &body options)
  "Define a functional record type with named fields.
   ...
   Options can include automatic type class instance derivation:
   (:deriving show eq-class hash-class)
  ...
  (let* ((constructor (intern (format nil "MAKE-~A" name)))
         ...
         (class-name (intern (format nil "%~A%" name)))
         (accessors (mapcar (lambda (field)
                              (intern (format nil "~A-~A" name field)))
                            fields))
         (field-keywords (mapcar (lambda (field) (intern (string field) :keyword))
                                 fields))
         (implementations '())
         (deriving-list nil))

    ;; Parse options for protocol implementations and deriving
    (dolist (option options)
      (when (listp option)
        (case (first option)
          ...
          (:deriving
           (setf deriving-list (rest option))))))
    ...
    `(progn
       ...
       ,@(when deriving-list
           (tc-std:generate-deriving-instances name class-name fields accessors deriving-list))
