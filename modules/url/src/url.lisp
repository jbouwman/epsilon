...
(defpackage epsilon.url
  ...
  (:export
   ...
   ;; Map-based query parameter handling
...
(defun coerce-param-value (value)
  "Coerce a query parameter value to a string.
   Returns nil for nil values, passes strings through,
   converts other types via princ-to-string."
  (cond
    ((null value) nil)
    ((stringp value) value)
    (t (princ-to-string value))))

(defun build-query-string-from-map (params)
  "Build a percent-encoded query string from a map.
   Keys and values are encoded per RFC 3986 via url-encode-component.
   Non-string values are coerced; nil values are skipped.

   Example:
     (build-query-string-from-map
       (map:make-map \"q\" \"hello world\" \"limit\" 20 \"debug\" nil))
     => \"q=hello%20world&limit=20\""
  (let ((parts '()))
    (map:each (lambda (k v)
                (let ((coerced (coerce-param-value v)))
                  (when coerced
                    (push (concatenate 'string
                             (url-encode-component (coerce-param-value k))
                             "="
                             (url-encode-component coerced))
                          parts))))
              params)
    (when parts
      (reduce #'join-with-ampersand (nreverse parts)))))

(defun url-with-params (base-url params)
  "Append query parameters from a map to a base URL string.
   Returns a new URL string with encoded query parameters.
   If the base URL already has query parameters, new ones are appended.
   Nil values are skipped. Non-string values are coerced.

   Examples:
     (url-with-params \"https://example.com/search\"
                      {\"q\" \"hello world\" \"limit\" 20})
     => \"https://example.com/search?q=hello%20world&limit=20\"

     (url-with-params \"/items?sort=date\" {\"page\" 2})
     => \"/items?sort=date&page=2\""
  (let ((query-string (build-query-string-from-map params)))
    (if (or (null query-string) (string= query-string ""))
        base-url
        (if (find #\? base-url)
            (concatenate 'string base-url "&" query-string)
            (concatenate 'string base-url "?" query-string)))))

(defun parse-query-string-to-map (query-string)
  "Parse a query string into a map (inverse of build-query-string-from-map)."
  (if (or (null query-string) (string= query-string ""))
      map:+empty+
      (map:from-pairs (parse-query-string query-string))))
