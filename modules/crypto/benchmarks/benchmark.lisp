...
(defpackage epsilon.crypto.benchmark
  (:use :cl)
  (:import
   (epsilon.crypto.sha256 native-sha256)
   (epsilon.crypto.sha512 native-sha512)
   (epsilon.crypto.hmac native-hmac)
   (epsilon.crypto.hkdf native-hkdf)
   (epsilon.crypto.aes-gcm native-aes-gcm)
   (epsilon.crypto.chacha20-poly1305 native-chacha)
   (epsilon.crypto.curve25519 native-x25519)
   (epsilon.crypto.ed25519-sign native-ed-sign)
   (epsilon.crypto.ecdsa native-ecdsa)
   (epsilon.crypto.drbg native-drbg)
   (epsilon.crypto.native bridge)
   (epsilon.crypto.modular mod-arith)
   (epsilon.crypto.rsa rsa)
   (epsilon.net net))
  (:export #:run-benchmarks))

(in-package :epsilon.crypto.benchmark)
...
(defun main ()
  "Run crypto primitive benchmarks from the command line."
