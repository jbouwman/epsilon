...
(in-package :epsilon.crypto.bridge-tests)
              ...
              (lambda ()
                (handler-case
                    (multiple-value-bind (fd tcp) (accept-connection listener)
                      ...
                      (setf server-conn (bridge:tls-accept tcp server-ctx)))
        ...
        (unwind-protect
            (progn
              ...
              (multiple-value-bind (fd tcp) (connect-to-loopback port)
                (declare (ignore fd))
                (setf client-tcp tcp)
                (let ((client-conn (bridge:tls-connect tcp client-ctx
                                                        :hostname "localhost")))
                  ;; Wait for server
