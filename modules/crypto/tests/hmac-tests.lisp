...
;;; ---------------------------------------------------------------------------
;;;
;;; HMAC-SHA1 was added to this module to support legacy applications
;;; (TOTP/HOTP, older PKCS#12, certain TLS 1.2 cipher suites). The two
;;; vectors below are RFC 2202 test cases 1 and 7, which together cover
;;; the short-key and long-key (>block size) code paths.

(deftest test-hmac-sha1-rfc2202-case1
  "RFC 2202 Test Case 1: 20-byte 0x0b key, 'Hi There'."
  (let* ((key (make-repeated-byte #x0b 20))
         (data (sb-ext:string-to-octets "Hi There" :external-format :ascii))
         (expected (hex-to-bytes "b617318655057264e28bc0b6fb378c8ef146be00")))
    (assert-true (ct:ct-equal (hmac:hmac-sha1 key data) expected))))

(deftest test-hmac-sha1-rfc2202-case7
  "RFC 2202 Test Case 7: 80-byte 0xaa key (longer than block size),
   exercises the 'hash the key first' branch."
  (let* ((key (make-repeated-byte #xaa 80))
         (data (sb-ext:string-to-octets
                "Test Using Larger Than Block-Size Key and Larger Than One Block-Size Data"
                :external-format :ascii))
         (expected (hex-to-bytes "e8e99d0f45237d786d6bbaa7965c7808bbff1a91")))
    (assert-true (ct:ct-equal (hmac:hmac-sha1 key data) expected))))

(deftest test-hmac-dispatch-sha1
  "The generic `hmac` function dispatches :sha1 correctly."
  (let* ((key (make-repeated-byte #x0b 20))
         (data (sb-ext:string-to-octets "Hi There" :external-format :ascii))
         (expected (hex-to-bytes "b617318655057264e28bc0b6fb378c8ef146be00")))
    (assert-true (ct:ct-equal (hmac:hmac :sha1 key data) expected))))
