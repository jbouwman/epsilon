...
(defpackage epsilon.crypto.ed25519
  (:use :cl)
  (:import-from #:epsilon.crypto.fe-25519
   #:+p-25519+ #:fe-add #:fe-sub #:fe-mul #:fe-sqr #:fe-inv)
  (:import-from #:epsilon.crypto.ct
   #:ct-select-int)
  (:export
   ;; Point operations
   #:ed-point
...
(in-package :epsilon.crypto.ed25519)
...
(defun ed-scalar-mul (n point)
  "Compute n * point using right-to-left double-and-add.
   ...
   leaks the bit length of N, and the conditional add leaks each
   individual bit. Suitable only for verification (where N is the
   public scalar). Signing paths must use `ed-scalar-mul-ct`."
...
(defun %ct-select-ed-point (bit p-if-1 p-if-0)
  "Branchless select between two extended-coordinate points. Field
   elements are non-negative bignums in [0, p), so the per-coordinate
   `ct-select-int` does the right thing."
  (declare (type (integer 0 1) bit))
  (make-ed-point (ct-select-int bit (ed-point-x p-if-1) (ed-point-x p-if-0))
                 (ct-select-int bit (ed-point-y p-if-1) (ed-point-y p-if-0))
                 (ct-select-int bit (ed-point-z p-if-1) (ed-point-z p-if-0))
                 (ct-select-int bit (ed-point-tt p-if-1) (ed-point-tt p-if-0))))

(defun ed-scalar-mul-ct (n point &key (bits 256))
  "Constant-time scalar multiplication on Ed25519.

   Performs BITS iterations regardless of the value of N: each
   iteration doubles the running result, computes the candidate
   sum (running + POINT) using the unified Hisil et al. formula
   (which is well-defined for every input pair, including the
   identity), and selects between the sum and the just-doubled
   running result via branchless mask-and-XOR over the four
   extended coordinates.

   BITS defaults to 256, comfortably above the bit length of the
   group order ℓ = 2^252 + 27742317777372353535851937790883648493
   so the iteration count never reveals a smaller scalar.

   Use this routine in EdDSA signing (private-key derivation,
   per-signature R = r·B) where N is secret. The non-CT
   `ed-scalar-mul` is faster and is appropriate for verification
   paths where the scalars are public."
  (let ((result (ed-neutral))
        (k (mod n +l+)))
    (loop for i from (1- bits) downto 0
          do (setf result (ed-point-double result))
             (let* ((bit (ldb (byte 1 i) k))
                    (candidate (ed-point-add result point)))
               (setf result (%ct-select-ed-point bit candidate result))))
    result))
