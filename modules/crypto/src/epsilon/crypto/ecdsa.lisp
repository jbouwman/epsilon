...
(defpackage epsilon.crypto.ecdsa
  (:use :cl)
  (:import
...
(in-package :epsilon.crypto.ecdsa)
...
(defun ecdsa-public-key-from-private (private-key)
  "Compute the P-256 public key from an integer private key.
   Returns an ec-p256 point. Uses constant-time scalar multiplication
   because the input scalar is the secret key."
  (ec:p256-scalar-mul-ct private-key (ec:p256-base-point)))

(defun ecdsa-sign (private-key message &key (hash :sha256))
  "Sign MESSAGE with private key (integer).
   Returns (values r s) as integers.

   Both scalar multiplications that touch secret values --
   k * G for the per-signature nonce, and the implicit private-key
   multiplication via the (z + r*d) computation -- run through the
   CT path. The scalar mul of u1 * G + u2 * Q in verification stays
   on the faster non-CT path because its scalars are public."
  ...
  (let* ((z-bytes (sha256:sha256 message))
         ...
         ;; R = k * G (CT: k is the per-signature secret nonce)
         (big-r (ec:p256-scalar-mul-ct k (ec:p256-base-point))))
    ;; r = R.x mod n
