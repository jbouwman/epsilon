...
(defpackage epsilon.crypto.asn1
  (:use :cl)
  (:export
   ...
   #:der-encode-tagged
   ;; Safe decoding with input limits
   #:der-decode-safe
   #:+max-input-size+
   #:+max-node-count+
   ;; Decode error condition
   #:asn1-decode-error
   #:asn1-decode-error-reason))

(in-package :epsilon.crypto.asn1)

;;; ---------------------------------------------------------------------------
;;; Decode error condition
;;; ---------------------------------------------------------------------------

(define-condition asn1-decode-error (error)
  ((reason :initarg :reason :reader asn1-decode-error-reason))
  (:report (lambda (c s)
             (format s "ASN.1 decode error: ~A" (asn1-decode-error-reason c))))
  (:documentation
   "Signaled when the DER decoder encounters truncated, overlength, or
structurally invalid ASN.1 encoding."))

(defun asn1-error (fmt &rest args)
  "Signal ASN1-DECODE-ERROR with a formatted reason string."
  (error 'asn1-decode-error :reason (apply #'format nil fmt args)))
...
;; Input size and node budget for untrusted data
(defconstant +max-input-size+ #.(* 16 1024 1024)
  "Maximum input size for der-decode-safe (16 MiB).")
(defconstant +max-node-count+ 65536
  "Maximum ASN.1 nodes per der-decode-safe call.")
(defvar *node-budget* nil
  "When non-NIL, a cons cell whose car is decremented per node.
   Used by der-decode-safe to enforce a parse budget.")
...
(defun read-tag (data pos)
  ...
  (when (>= pos (length data))
    (asn1-error "ASN.1: unexpected end of data reading tag"))
  (let* ((byte (aref data pos))
    ...
    (if (= tag-number #x1F)
        ...
        (let ((n 0))
          (loop
            (when (>= pos (length data))
              (asn1-error "ASN.1: unexpected end of data in long tag"))
            (let ((b (aref data pos)))
              (incf pos)
              (setf n (logior (ash n 7) (logand b #x7F)))
              (when (> n #xFFFFFFFF)
                (asn1-error "ASN.1: tag number exceeds 32 bits"))
              (unless (logbitp 7 b)
                (return))))
...
(defun read-length (data pos)
  ...
  (when (>= pos (length data))
    (asn1-error "ASN.1: unexpected end of data reading length"))
  (let ((byte (aref data pos)))
    (incf pos)
    (cond
      ((not (logbitp 7 byte))
      ...
      ((zerop (logand byte #x7F))
       (asn1-error "ASN.1: indefinite length not allowed in DER"))
      (t
       ;; Long form
       (let ((num-bytes (logand byte #x7F)))
         (when (> num-bytes 4)
           (asn1-error "ASN.1: length too large (> 4 bytes)"))
         (let ((len 0))
           (loop for i from 0 below num-bytes
                 do (when (>= pos (length data))
                      (asn1-error "ASN.1: unexpected end of data in length"))
                    (setf len (logior (ash len 8) (aref data pos)))
                    (incf pos))
           ;; DER: must use shortest encoding
           (when (and (= num-bytes 1) (< len 128))
             (asn1-error "ASN.1: DER violation: non-minimal length encoding"))
           (values len pos)))))))
...
(defun der-decode (data &key (start 0) (depth 0))
  ...
  (when (> depth +max-depth+)
    (asn1-error "ASN.1: maximum nesting depth exceeded"))
  (when *node-budget*
    (when (<= (car *node-budget*) 0)
      (asn1-error "ASN.1: node budget exceeded"))
    (decf (car *node-budget*)))
  (multiple-value-bind (tag class constructed new-pos) (read-tag data start)
    (multiple-value-bind (len val-start) (read-length data new-pos)
      (when (> (+ val-start len) (length data))
        (asn1-error "ASN.1: value extends beyond data (need ~d bytes at ~d, have ~d)"
            ...
            (let ((children nil)
              ...
              (loop while (< pos val-end)
                    do (multiple-value-bind (child child-end)
                         ...
                         (setf pos child-end)))
              (unless (= pos val-end)
                (asn1-error "ASN.1: constructed value length mismatch"))
...
(defun der-decode-sequence-contents (tlv)
  ...
  (unless (asn1-tlv-constructed-p tlv)
    (asn1-error "ASN.1: expected constructed type"))
  (asn1-tlv-value tlv))

(defun der-decode-safe (data &key (start 0)
                                   (max-size +max-input-size+)
                                   (max-nodes +max-node-count+))
  "Decode one DER TLV with input-size and node-count guards.
   Use for untrusted input (certificates, OCSP responses, etc.)."
  (when (> (length data) max-size)
    (asn1-error "ASN.1: input exceeds ~D byte limit" max-size))
  (let ((*node-budget* (cons max-nodes nil)))
    (der-decode data :start start)))
...
(defun decode-der-integer (bytes)
  ...
  (if (zerop (length bytes))
      (asn1-error "ASN.1: empty INTEGER encoding")
...
(defun der-encode-oid (oid-components)
  ...
  (when (< (length oid-components) 2)
    (asn1-error "ASN.1: OID must have at least 2 components"))
...
(defun decode-oid-value (bytes)
  ...
  (when (zerop (length bytes))
    (asn1-error "ASN.1: empty OID encoding"))
          ...
          do (let ((n 0))
               (loop
                 (when (>= pos (length bytes))
                   (asn1-error "ASN.1: truncated OID component"))
