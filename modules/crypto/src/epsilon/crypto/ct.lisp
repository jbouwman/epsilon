...
(defun ct-equal-int (a b &key (start1 0) (end1 nil) (start2 0) (end2 nil))
  "Compare two byte arrays in constant time. Returns 1 if equal, 0 otherwise.
   Integer return avoids caller branches on a boolean."
  (declare (type (simple-array (unsigned-byte 8) (*)) a b)
           (type fixnum start1 start2)
           (optimize (speed 3) (safety 1)))
  (let* ((end1 (or end1 (length a)))
    ...
    (let ((diff (logxor len1 len2)))
      ...
      (loop for i fixnum from 0 below len1
            do (let ((b1 (aref a (+ start1 i)))
                 ...
                 (declare (type (unsigned-byte 8) b1 b2))
                 (setf diff (logior diff (logxor b1 b2)))))
      ;; Fold diff to a single bit: 0 if zero, 1 if nonzero
      (let* ((d (logior diff (ash diff -1)))
             (d (logior d (ash d -2)))
             (d (logior d (ash d -4)))
             (d (logior d (ash d -8)))
             (d (logior d (ash d -16)))
             (bit0 (logand d 1)))
        (declare (type fixnum d bit0))
        (logxor bit0 1)))))
...
(defun ct-select-int (bit a b)
  "Constant-time select between two non-negative integers (including
   bignums). BIT must be 0 or 1; returns A when BIT=1, B when BIT=0,
   without an algorithmic branch on BIT.

   The caller is responsible for ensuring BIT is exactly 0 or 1
   (e.g. via `(ldb (byte 1 i) secret)`); the implementation does not
   guard against other values. For arbitrary truthy/falsy operands
   use the boolean-flavoured `ct-select` (which is fixnum-only)."
  (declare (type (integer 0 1) bit)
           (type (integer 0) a b))
  (let* ((mask (- bit))           ; 0 -> 0, 1 -> -1 (all-ones in two's complement)
         (delta (logxor a b)))
    (logxor b (logand mask delta))))
