...
(defpackage epsilon.crypto.primitives
  (:use :cl)
  (:export
   ;; Constants
   #:+u32-mask+
   ...
   #:u64-to-bytes-be
   #:read-le64
   ;; Byte-vector coercion
   #:ensure-byte-vector
   ;; Big-endian integer / byte-vector conversion (variable width)
   #:int-to-be-bytes
   #:be-bytes-to-int
   #:bits2int))

(in-package :epsilon.crypto.primitives)
...
(defun ensure-byte-vector (x)
  "Coerce X to a SIMPLE-ARRAY of (UNSIGNED-BYTE 8). Strings are UTF-8 encoded.
   Used by callers that accept either pre-allocated byte vectors or convenient
   collections (string, list, generic vector)."
  (etypecase x
...
(defun int-to-be-bytes (value width)
  "Encode VALUE as a big-endian byte vector of length WIDTH. Used by EC
   point encoding and ECDSA signature serialization where the encoded
   width is curve-specific (32 / 48 / 66)."
  (let ((b (make-array width :element-type '(unsigned-byte 8) :initial-element 0))
        (v value))
    (loop for i from (1- width) downto 0
          do (setf (aref b i) (logand v #xFF))
             (setf v (ash v -8)))
    b))

(defun be-bytes-to-int (bytes)
  "Decode a big-endian byte vector to a non-negative integer."
  (let ((n 0))
    (loop for b across bytes do (setf n (logior (ash n 8) b)))
    n))

(defun bits2int (bytes qlen)
  "RFC 6979 §2.3.2 bits2int: decode BYTES as a big-endian integer, then
   right-shift if the bit-length of BYTES exceeds QLEN. Used by ECDSA
   deterministic-k derivation."
  (let ((blen (* 8 (length bytes)))
        (x (be-bytes-to-int bytes)))
    (if (> blen qlen) (ash x (- qlen blen)) x)))
