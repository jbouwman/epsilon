...
(defpackage epsilon.crypto.hmac
  (:use :cl)
  (:import
...
(in-package :epsilon.crypto.hmac)

(defconstant +hmac-sha1-output-size+ 20)

(defun hmac-sha1 (key message &key (key-start 0) (key-end nil)
                                   (msg-start 0) (msg-end nil))
  "Compute HMAC-SHA1(key, message) per RFC 2104. Returns a 20-byte tag.
   SHA-1 is retained only for legacy compatibility (HOTP/TOTP, older
   PKCS#12, some TLS 1.2 cipher suites). New designs should use SHA-256
   or stronger."
  (declare (type (simple-array (unsigned-byte 8) (*)) key message))
  (let* ((key-end (or key-end (length key)))
         (msg-end (or msg-end (length message)))
         (block-size sha1:+sha1-block-size+)
         (actual-key (if (> (- key-end key-start) block-size)
                         (sha1:sha1 (subseq key key-start key-end))
    ...
    (ct:with-secure-buffers ((ipad block-size)
      ...
      (let* ((inner-state (sha1:make-sha1-state)))
        (sha1:sha1-update inner-state ipad)
        (sha1:sha1-update inner-state message :start msg-start :end msg-end)
        (let ((inner-hash (sha1:sha1-finalize inner-state))
              (outer-state (sha1:make-sha1-state)))
          (sha1:sha1-update outer-state opad)
          (sha1:sha1-update outer-state inner-hash)
          (sha1:sha1-finalize outer-state))))))
...
(defun hmac (hash-algorithm key message &key (key-start 0) (key-end nil)
  ...
  "Compute HMAC with the specified hash algorithm.
   HASH-ALGORITHM is one of :sha1, :sha256, :sha384, :sha512."
  (ecase hash-algorithm
    (:sha1   (hmac-sha1   key message :key-start key-start :key-end key-end
                                      :msg-start msg-start :msg-end msg-end))
