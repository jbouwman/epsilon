...
(defpackage epsilon.crypto.ed25519-sign
  (:use :cl)
  (:import
...
(in-package :epsilon.crypto.ed25519-sign)
...
(defun ed25519-public-key-from-private (private-key)
  ...
  (let* ((h (sha512:sha512 private-key))
         ...
         ;; A = a * B (CT scalar mul: a is derived from the private key)
         (big-a (ed:ed-scalar-mul-ct a (ed:ed-base-point))))
...
(defun ed25519-sign (private-key message)
  ...
  (let* ((h (sha512:sha512 private-key))
         ...
         ;; A = a * B (public key point) -- CT: a derived from private key
         (big-a (ed:ed-scalar-mul-ct a (ed:ed-base-point)))
         (pub-bytes (ed:ed-point-encode big-a))
         ;; r = SHA-512(prefix || message) mod L
         (r (sha512-modl (concat-bytes prefix message)))
         ;; R = r * B (per-signature secret nonce -- CT mandatory)
         (big-r (ed:ed-scalar-mul-ct r (ed:ed-base-point)))
