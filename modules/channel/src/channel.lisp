...
(defstruct (channel
  ...
  (lock (lock:make-lock "channel") :type lock:lock)
  ...
  (let ((deadline (when timeout (+ (get-universal-time) timeout))))
    (lock:with-lock ((channel-lock channel))
      (loop
        ...
        (cond
          ;; Receiver is waiting -- buffer the value and wake it
          ((channel-receivers channel)
          ...
          (t
           ...
           (let ((condition (cv:make-condition-variable)))
             (push (cons value condition) (channel-senders channel))
             (cv:condition-wait condition (channel-lock channel)
...
(defun try-send (channel value)
  ...
  (lock:with-lock ((channel-lock channel))
    (cond
      ;; Receiver waiting -- buffer the value and wake it
      ((channel-receivers channel)
       (let ((receiver (pop (channel-receivers channel))))
         (setf (channel-buffer channel)
               (append (channel-buffer channel) (list value)))
...
(defvar *task-run-hook* nil
  "When non-nil, a function (fn thunk &key name) that creates a lightweight
task (e.g., coroutine) instead of an OS thread. Set by the coroutine
scheduler to make spawn, async, and related operations use coroutines.")
  ...
  (let ((channel (make-channel :capacity buffer-size))
                    ...
                    (multiple-value-bind (item more-p) (stream-next stream)
                      (if more-p
                          ...
                          (progn
                            ...
                            (return))))))))
      (setf producer-handle
            (if *task-run-hook*
                (funcall *task-run-hook* thunk :name "stream-buffer-producer")
                (thread:make-thread thunk :name "stream-buffer-producer"))))
    ...
    (make-data-stream
     ...
     :close-fn (lambda ()
                 (stream-close stream)
                 (close-channel channel)
                 (when (and producer-handle
                            (typep producer-handle 'thread:thread))
                   (thread:join-thread producer-handle))))))
...
(defstruct (promise (:constructor %make-promise)
  ...
  (error-value nil)
  (lock (lock:make-lock "promise-lock"))
  (condition (cv:make-condition-variable)))
...
(defmacro async (&body body)
  "Execute BODY asynchronously, returning a promise.
   ...
   When a scheduler is active (*task-run-hook* is set), creates a coroutine
   instead of an OS thread. Falls back to thread:make-thread otherwise.
               ...
               (lambda ()
                 (handler-case
                     (let ((result (progn ,@body)))
                       (lock:with-lock ((promise-lock ,promise-var))
                         (setf (promise-value ,promise-var) result)
                         (setf (promise-ready ,promise-var) t)
                         (cv:condition-broadcast (promise-condition ,promise-var))))
                   (error (e)
                     (lock:with-lock ((promise-lock ,promise-var))
                       ...
                       (cv:condition-broadcast (promise-condition ,promise-var))))))))
         (if *task-run-hook*
             (funcall *task-run-hook* thunk :name "async-worker")
             (thread:make-thread thunk :name "async-worker")))
       ,promise-var)))
...
(defmacro spawn (&body body)
  ...
  (let ((thunk (gensym "THUNK")))
    `(let ((,thunk (lambda () ,@body)))
       (if *task-run-hook*
           (funcall *task-run-hook* ,thunk :name "goroutine")
           (thread:make-thread ,thunk :name "goroutine")))))
...
(defun timeout-channel (seconds)
  ...
  (let ((ch (make-channel :capacity 1)))
    (let ((thunk (lambda ()
                   ...
                   (close-channel ch))))
      (if *task-run-hook*
          (funcall *task-run-hook* thunk :name "timeout-channel")
          (thread:make-thread thunk :name "timeout-channel")))
    ch))
