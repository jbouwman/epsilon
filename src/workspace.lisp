...
;;;;
;;;; Workspace file format (workspace.plist at repository root):
;;;;
;;;;   (:name "my-project"
...
;;;;               Each referenced directory must contain a workspace.plist file
...
(define-condition workspace-not-found-error (workspace-error)
  ()
  (:report (lambda (condition stream)
             (format stream "Workspace file not found: ~A/workspace.plist"
                     (workspace-error-path condition)))))
...
(defclass workspace ()
  ((name :initarg :name
   ...
   (scan-dirs :initarg :scan-dirs
              :reader workspace-scan-dirs
              :initform nil
              :documentation "List of relative paths to directories to scan for modules")
...
(defmethod print-object ((ws workspace) stream)
  (print-unreadable-object (ws stream :type t)
    (let ((scan-count (length (workspace-scan-dirs ws))))
      (format stream "~A (~A modules~:[~;, ~:*~A scan dirs~], ~A refs)"
              (workspace-name ws)
              (length (workspace-module-paths ws))
              (when (plusp scan-count) scan-count)
              (length (workspace-workspace-refs ws))))))

;;; Validation

(defun validate-workspace-metadata (metadata &optional filepath)
  "Validate workspace.plist metadata. Returns NIL if valid, or list of error messages.
   FILEPATH is optional, used for better error messages in the future."
  (declare (ignore filepath))
  (let ((errors nil)
        (valid-keys '(:name :scan :modules :workspaces)))
    ...
    ;; Validate :scan if present
    (let ((scan (getf metadata :scan)))
      (when scan
        (unless (listp scan)
          (push (format nil ":scan must be a list, got ~A" (type-of scan)) errors))
        (when (listp scan)
          (loop for dir in scan
                for index from 0
                unless (stringp dir)
                  do (push (format nil ":scan entry ~D must be a string, got ~A"
                                  index (type-of dir)) errors)))))
...
(defun load-workspace (workspace-dir &key (resolve t) (visited nil))
  "Load a workspace from a directory containing workspace.plist.

   WORKSPACE-DIR - Path to directory containing workspace.plist
  ...
  (let* ((ws-path (path:ensure-path workspace-dir))
         (ws-file (path:path-join ws-path "workspace.plist"))
    ...
    (let* ((metadata (handler-case
      ...
      (let ((workspace (make-instance 'workspace
                                      ...
                                      :scan-dirs (or (getf metadata :scan) nil)
...
(defun workspace-all-module-paths (workspace)
  "Get all module paths from this workspace and all referenced workspaces.
   ...
   Preserves order: scanned modules first, then explicit modules, then referenced workspace modules."
  (let ((results nil)
        (ws-name (workspace-name workspace))
        (ws-location (workspace-location workspace)))
    ;; Scan directories first
    (dolist (rel-dir (workspace-scan-dirs workspace))
      (let ((abs-dir (path:path-string (path:path-join ws-location rel-dir))))
        (when (probe-file abs-dir)
          (dolist (sub-path (path:list-directory (path:ensure-path abs-dir) :type :directories))
            (let ((sub-str (path:path-string (path:ensure-path sub-path))))
              (let ((module-file (path:path-string
                                  (path:path-join sub-str "module.plist"))))
                (when (probe-file module-file)
                  (push (cons ws-name sub-str) results))))))))

    ;; Add explicit modules
    (dolist (rel-path (workspace-module-paths workspace))
      (let ((abs-path (path:path-string
                       (path:path-join ws-location rel-path))))
        (push (cons ws-name abs-path) results)))
            ...
            (when (probe-file module-file)
              (handler-case
                  (let* ((metadata (with-open-file (stream module-file)
                    ...
                    (unless (mmap:contains? seen-names module-name)
                      (mmap:put! seen-names module-name t)
                      (let ((module-info
...
(defun make-workspace-environment (workspace-path &key config)
  "Create a build environment from a workspace file.
   ...
   WORKSPACE-PATH - Path to directory containing workspace.plist
...
(defun find-workspace-root (&optional (start-dir (path:current-directory)))
  "Search upward from START-DIR for a directory containing workspace.plist.
   Returns the directory path if found, NIL otherwise."
  (let ((current (path:ensure-path start-dir)))
    (loop
      (let ((ws-file (path:path-join current "workspace.plist")))
...
(defun workspace-module-names (workspace)
  ...
  (mapcar #'car
          (map:to-alist (resolve-workspace-modules workspace))))
