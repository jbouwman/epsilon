...
(defun set-option (req var value &optional (type :uint32))
  (sys.ffi:with-foreign-object (buf type)
    (setf (sys.ffi:mem-aref buf type) value)
    (let ((ret (winhttp::%set-option req
                                     var
                                     buf
                                     (sys.ffi:foreign-type-size type))))
      (unless ret
  ...
  (etypecase content
    (cons
     (cond (multipart-p
           ...
           (form-urlencoded-p
            (values
             (string-to-u8 (url-encode-params content))
             "application/x-www-form-urlencoded"))
           (t
            (error "Can't convert a CONS content"))))
    (string
     (values (string-to-u8 content)
