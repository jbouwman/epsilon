;;;; Epsilon reader syntax extensions
;;;;
...
(defun parse-anon-fn (stream char arg)
  ...
  (let* ((body (read stream t nil t))
    ...
    (cond
      ...
      ;; Add (declare (ignorable ...)) to suppress warnings for unused args
      (t
...
(defun initialize-epsilon-readtable ()
  ...
  (unless *epsilon-readtable*
    ...
    ;; Annotations: #@(:key value) or #@:keyword
    (set-dispatch-macro-character #\# #\@ #'read-annotation *epsilon-readtable*))
...
(defun enable-epsilon-syntax ()
  "Enable Epsilon reader syntax extensions.
   ...
   - #@(:key val) form   -> Annotation binding
   - #@:keyword form     -> Bare annotation (shorthand for #@(:keyword t))
