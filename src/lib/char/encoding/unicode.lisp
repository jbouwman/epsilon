     ...
     (loop with di fixnum = d-start
           ...
           (macrolet ((set-octet (offset value)
             ...
             (cond
               ...
               ;; 2 octets
               ((< code #x800)
                (set-octet 0 (logior #xc0 (ash code -6)))
                (set-octet 1 (logior #x80 (logand code #x3f)))
                (incf di 2))
               ;; 3 octets
               ((< code #x10000)
                (set-octet 0 (logior #xe0 (ash code -12)))
                (set-octet 1 (logior #x80 (logand #x3f (ash code -6))))
                (set-octet 2 (logior #x80 (logand code #x3f)))
                (incf di 3))
               ;; 4 octets
               (t
                (set-octet 0 (logior #xf0 (logand #x07 (ash code -18))))
                (set-octet 1 (logior #x80 (logand #x3f (ash code -12))))
                (set-octet 2 (logior #x80 (logand #x3f (ash code -6))))
...
(define-decoder :utf-8 (getter src-type setter dest-type)
  `(named-lambda utf-8-decoder (src start end dest d-start)
     ...
     (let ((u2 0) (u3 0) (u4 0) (u5 0) (u6 0))
       (declare (type u8 u2 u3 u4 u5 u6))
       (loop for di fixnum from d-start
             for i fixnum from start below end
             for u1 of-type u8 = (,getter src i) do
     ...
     (loop with nchars fixnum = 0
           ...
           (let* ((octet (,getter seq i))
                  (noctets (cond ((< octet #x80) 1)
                                 ...
                                 ((< octet #xf0) 3)
                                 (t 4))))
             (declare (type u8 octet) (fixnum noctets))
             (cond
               ((> (+ i noctets) end)
                ...
                (let* ((u1 octet)
                       ...
                       (inc (or (and (> noctets 1)
                                ...
                                (and (= noctets 2)
                                     (not (logior u2 #x40)))
                                (and (= noctets 3)
                                     (not (and (< (logxor u2 #x80) #x40)
                                               (< (logxor u3 #x80) #x40)
                                               (or (>= u1 #xe1) (>= u2 #xa0))
                                               (or (/= u1 #xed) (< u2 #xa0) (> u2 #xbf)))))
                                (and (= noctets 4)
                                     (not
                                      (and (< (logxor u2 #x80) #x40)
                                           (< (logxor u3 #x80) #x40)
                                           (< (logxor u4 #x80) #x40)
                                           (or (>= u1 #xf1) (>= u2 #x90))))))))
           ...
           `(named-lambda ,',encoder-name (src start end dest d-start)
              ...
              (loop with di fixnum = d-start
                ...
                (cond ((< high-bits 0)
                      ...
                      (t
                        ,,(make-setter-form
                              'setter ''(logior #xd800 (ash high-bits -10))
                            ''dest ''di)
                        ,,(make-setter-form
                              'setter ''(logior #xdc00 (logand high-bits #x3ff))
                            ''dest ''(+ di 2))
                        (incf di 4)))
                finally (return (the fixnum (- di d-start))))))
