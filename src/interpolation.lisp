...
;;;;
;;;; Python-style string interpolation with #~"..." syntax.
...
(defun parse-interpolated-string (stream char arg)
  "Parse #~\"...\" interpolated string syntax.
   ...
   Examples:
     #~\"Hello, ~{name}!\"       => (concatenate 'string \"Hello, \" (princ-to-string name) \"!\")
     #~\"Sum is ~{(+ a b)}\"     => (concatenate 'string \"Sum is \" (princ-to-string (+ a b)))
  ...
  (let ((parts '())
    ...
    (let ((reversed-parts (nreverse parts)))
      (cond
        ...
        ;; Multiple parts - generate concatenate call
        (t
         `(concatenate 'string
           ,@(mapcar (lambda (part)
                       (if (stringp part)
...
(defun install-interpolation-reader ()
  ...
  ;; Modify the current readtable in-place rather than replacing it.
  ;; This is necessary because LOAD binds *readtable* dynamically,
  ;; so SETF *readtable* would only affect the local binding.
...
(defmacro with-interpolation-reader (&body body)
  "Execute BODY with interpolation reader extensions enabled.
   Restores original dispatch function after BODY completes.
  ...
  (let ((saved-fn (gensym "SAVED-FN"))
        (saved-rt (gensym "SAVED-RT")))
    `(let ((,saved-fn (get-dispatch-macro-character #\# #\~ *readtable*))
           (,saved-rt *readtable*))
       (unwind-protect
            (progn
              (install-interpolation-reader)
              ,@body)
         ;; Restore original dispatch function (or remove if there wasn't one)
         (set-dispatch-macro-character #\# #\~ ,saved-fn ,saved-rt)
         (setf *interp-original-readtable* nil)
         (setf *interp-readtable* nil)))))

;;; Enable interpolation syntax immediately when this file is loaded.
;;; This allows all subsequently loaded files to use #~"..." syntax.
(install-interpolation-reader)
